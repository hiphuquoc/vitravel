@php
    $rows = $oldData ?? null;
    if ($rows === null) {
        $rows = ($data ?? collect())->map(function ($row) use ($locale) {
            $pkg = $row->package;
            $translation = $pkg?->translation($locale);
...
<div class="adminFormSection adminFormSection--repeater repeater" data-repeater-container>
    <div class="adminFormSection_header">
        <div class="adminFormSection_header_info">
            <h2 class="adminFormSection_title">Du thuyền hiển thị trên trang chủ</h2>
            <p class="adminFormSection_description">Chọn chương trình du thuyền có sẵn. Nội dung card chỉnh trong mục Gói Cruise. Kéo thả để đổi thứ tự.</p>
        </div>
        <button type="button" class="adminFormSection_header_action" data-repeater-create>
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/>
            </svg>
            <span>Thêm du thuyền</span>
        </button>
    </div>
    <div class="adminFormSection_body">
        <div data-repeater-list="featured_cruises">
                ...
                <div class="adminFormRepeater_item adminFormRepeater_item--block" data-repeater-item>
                    ...
                    <div class="adminFormRepeater_item_content">
                        ...
                        @include('admin.components.formField', [
                            'label' => 'Chương trình du thuyền',
                            'name' => 'package_id',
                            'type' => 'select',
                            'value' => $row['package_id'] ?? '',
                            'options' => ['' => '— Chọn du thuyền —'] + $cruiseOptions,
                            'class' => 'adminFormGrid__full adminFormRepeater_item_field',
                        ])
                        @if (! empty($row['code']))
                            <p class="adminFormRepeater_item_field" style="font-size:13px;color:#64748b;margin:0;">
                                <strong>{{ $row['code'] }}</strong>
                                @if (! empty($row['title'])) — {{ $row['title'] }} @endif
                                @if (! empty($row['duration_days'])) · {{ $row['duration_days'] }} ngày @endif
                                @if (! empty($row['rating']) && (float) $row['rating'] > 0)
                                    · ★ {{ number_format((float) $row['rating'], 1) }} ({{ $row['review_count'] ?? 0 }})
                                @endif
                                @if (($row['status'] ?? '') !== 'published') · <em>nháp</em> @endif
                            </p>
                        @endif
                    </div>
