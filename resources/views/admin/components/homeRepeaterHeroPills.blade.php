@php
    $rows = $oldData ?? null;
    if ($rows === null) {
        $rows = ($data ?? collect())->map(fn ($pill) => [
            'id' => $pill->id,
            'target' => $pill->linkTargetValue(),
            'label' => $pill->translation($locale)?->label ?? '',
...
<div class="adminFormSection adminFormSection--repeater repeater" data-repeater-container>
    <div class="adminFormSection_header">
        <div class="adminFormSection_header_info">
            <h2 class="adminFormSection_title">Điều hướng nhanh trên slider</h2>
            <p class="adminFormSection_description">Pill link tới danh mục tour hoặc trang tour theo quốc gia. Kéo thả để đổi thứ tự.</p>
        </div>
                ...
                <div class="adminFormRepeater_item adminFormRepeater_item--block" data-repeater-item>
                    ...
                    <div class="adminFormRepeater_item_content">
                        ...
                        @include('admin.components.formField', [
                            ...
                            'type' => 'select',
                            'value' => $row['target'] ?? '',
                            'options' => ['' => '— Chọn danh mục / quốc gia —'] + $linkOptions,
                            'class' => 'adminFormGrid__full adminFormRepeater_item_field',
                        ])
                        @include('admin.components.formField', [
                            'label' => 'Nhãn hiển thị ('.strtoupper($locale).')',
                            'name' => 'label',
                            'value' => $row['label'] ?? '',
                            'helpText' => 'Để trống sẽ dùng tên danh mục / quốc gia.',
                        ...
                        @include('admin.components.formField', [
                            ...
                            'type' => 'checkbox',
                            'value' => $isActive,
                            'checkboxLabel' => 'Pill đang hiển thị trên slider',
                            'class' => 'adminFormRepeater_item_field',
                        ])
                    </div>
