...
class MediaService
{
    ...
    /** Chu?n hoá slug file: a-z0-9-, t?i ?a 80, không path traversal. */
    ...
    /**
     * Ch?n stem ch?a b? chi?m (file full + variants trên disk / DB, k? c? soft-delete).
     */
    ...
    public function mediaStemTaken(
        ...
        foreach ($paths as $path) {
            try {
                ...
                // Disk l?i t?m ? coi nh? ch?a chi?m, vòng allocate v?n an toàn nh? random fallback.
            }
        }
    ...
    /**
     * L?u file video thô (không resize) ? mp4 / webm / mov / ?
     */
    ...
    /**
     * Sync video file trên c?t FK (video_media_id).
     */
    ...
    /**
     * Xóa file trên disk (GCS/local) + hard-delete row media (không ?? soft-delete rác).
    ...
    /**
     * Ch? xóa file khi media không còn attachment / FK nào (?nh dùng chung th? vi?n).
     */
    ...
    /** B? ti?n t? projects/{code}/ kh?i path l?u trên disk. */
    ...
    /** Path ??y ?? (có projects/{code}/) cho folder key admin. */
    ...
    /**
     * Path prefix (có projects/{code}/) c?a các folder ?n ? dùng lo?i tr? khi list "T?t c?".
     *
     * @return list<string>
     */
    ...
    /** G?n / thay / xóa cover (media_attachments role=cover) theo media_id ?ã upload. */
    ...
    /**
     * ??ng b? gallery (media_attachments role=gallery) theo danh sách media_id.
     * Xóa toàn b? gallery c? r?i t?o l?i theo th? t? ? gi? media file (không xóa disk).
     *
    ...
    /**
     * G?n l?i gallery attachments t? media_id ?ã upload (attrs.gallery_media_ids / attrs.photos).
     */
    ...
    /** G?n / thay / xóa ?nh trên c?t FK (banner_media_id, ?). */
