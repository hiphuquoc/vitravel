<?php

/**
 * ============================================================================
 * DỮ LIỆU Hi Đà Lạt — profile `hidalat` (project:seed / migrate --seed)
 * ============================================================================
 *
 * Trang thông tin du lịch + dịch vụ + kết nối TOÀN DIỆN Đà Lạt (Lâm Đồng).
 * Một điểm đến duy nhất — "zones" thay "countries". Cụm "train" = xe khách,
 * limousine & xe liên tỉnh (không phải đảo). Sân bay DLI (Liên Khương) ~30km.
 *
 * Lưu trú: không seed catalogue trong file này (data từ tool cào riêng); giữ menu + stays_hub.
 *
 * ---------------------------------------------------------------------------
 * TAXONOMY TOUR (bắt buộc đồng bộ mọi hub — xem project/README.md §3):
 * - DANH MỤC = tour_categories type=region → khu vực / combo (zone hoặc ket-hop-*).
 *   Không chia theo số ngày; không đặt tên "Tour 1 ngày / 2–3 ngày…".
 * - CHỦ ĐỀ   = tour_categories type=theme  → gắn HUB zone:
 *   (A) thời lượng: tour-trong-ngay | 2N1D | 3N2D | 4N3D | từ 5 ngày
 *   (B) tính chất: gia đình, trăng mật, teambuilding, cuối tuần, insight địa phương
 *   — KHÔNG clone tên zone GEO.
 * - Package ↔ category/theme: nhiều–nhiều qua packageSlugs[] (+ minDays cho theme thời lượng).
 * - travel_styles: mã filter khớp chủ đề; không tạo trang SEO riêng.
 * - Cấm type=duration trên hub (trùng chủ đề thời lượng).
 * ---------------------------------------------------------------------------
 * Schema: project/README.md | Loader: App\Support\ProjectSeed::useProfile('hidalat')
 *
 * @return array<string, mixed>
 */

$__hidalatSeed = array(
    'meta' => array(
        'schema' => 1,
        'brand' => 'Hi Đà Lạt',
        'tagline' => 'Thành phố ngàn hoa — mùa xuân vĩnh cửu',
        'admin' => array(
            'email' => 'admin@hidalat.dev',
            'name' => 'Admin Hi Đà Lạt',
            'password' => '111111',
        ),
        'primary_domain' => 'hidalat.dev',
        'domains' => array('hidalat.dev', 'www.hidalat.dev', 'hidalat.com', 'www.hidalat.com', 'hidalat.vn', 'www.hidalat.vn'),
        'exported_at' => '2026-08-28T00:00:00+00:00',
    ),

    'price_guest_types' => array(
        array('code' => 'adult', 'sort' => 10, 'age_min' => 12, 'age_max' => 59, 'name' => array('vi' => 'Người lớn',



'en' => 'Adult')),
        array('code' => 'child', 'sort' => 20, 'age_min' => 2, 'age_max' => 11, 'name' => array('vi' => 'Trẻ em',



'en' => 'Child')),
        array('code' => 'senior', 'sort' => 30, 'age_min' => 60, 'age_max' => null, 'name' => array('vi' => 'Cao tuổi (60+)',



'en' => 'Senior (60+)')),
    ),

    'price_table_defaults' => array(
        'unit' => 'per_person',
        'notes' => 'Giá tham khảo theo người. Trẻ em và cao tuổi giảm theo bảng. Liên hệ để chốt báo giá chính xác.',
        'guest_multipliers' => array('adult' => 1, 'child' => 0.7, 'senior' => 0.85),
        'cluster_units' => array('stay' => 'per_room'),
        'periods' => array(
            array('kind' => 'year', 'label' => 'Giá năm {year}', 'is_promo' => false, 'priority' => 0),
            array('kind' => 'range', 'label' => 'Festival hoa {year}', 'starts_on' => '{year}-12-15', 'ends_on' => '{year}-12-31', 'is_promo' => true, 'priority' => 10, 'amount_multiplier' => 1.1),
        ),
    ),

    'content_tag_map' => array(
        'Ăn gì, uống gì?' => 'where-to-eat',
        'Ở đâu?' => 'where-to-stay',
        'Chơi gì, xem gì?' => 'what-to-do',
        'Di chuyển thế nào?' => 'how-to-get-there',
        'Mẹo du lịch' => 'travel-tips',
        'Chuyến đi thế nào?' => 'trip-report',
        'Chọn tour nào?' => 'which-tour',
    ),

    'content_tags' => array(
        'where-to-eat' => array('vi' => 'Ăn uống ở đâu?',



'en' => 'Where to eat & drink?'),
        'where-to-stay' => array('vi' => 'Ở đâu?',



'en' => 'Where to stay?'),
        'what-to-do' => array('vi' => 'Làm gì & xem gì?',



'en' => 'What to do & see?'),
        'how-to-get-there' => array('vi' => 'Di chuyển tới Đà Lạt thế nào?',



'en' => 'How to get to Da Lat?'),
        'travel-tips' => array('vi' => 'Mẹo du lịch',



'en' => 'Travel tips'),
        'trip-report' => array('vi' => 'Cảm nhận chuyến đi',



'en' => 'How was the trip?'),
        'which-tour' => array('vi' => 'Chọn tour nào?',



'en' => 'Which tour to choose?'),
    ),

    'travel_styles' => array(
        // (A) thời lượng chương trình — khớp chủ đề type=theme sort 0–4
        'day-trip' => array('vi' => 'Tour trong ngày', 'en' => 'Day trip'),
        '2n1d' => array('vi' => '2 ngày 1 đêm', 'en' => '2 days 1 night'),
        '3n2d' => array('vi' => '3 ngày 2 đêm', 'en' => '3 days 2 nights'),
        '4n3d' => array('vi' => '4 ngày 3 đêm', 'en' => '4 days 3 nights'),
        '5-plus-days' => array('vi' => 'Từ 5 ngày', 'en' => '5+ days'),
        // (B) tính chất / insight địa phương — khớp chủ đề type=theme sort 10+
        'cafe-nong-trai' => array('vi' => 'Cà phê & nông trại', 'en' => 'Coffee & farms'),
        'phieu-luu-canyon' => array('vi' => 'Phiêu lưu & canyoning', 'en' => 'Adventure & canyoning'),
        'cuoi-tuan-sai-gon' => array('vi' => 'Cuối tuần từ Sài Gòn', 'en' => 'Weekend from Saigon'),
        'gia-dinh-tre-em' => array('vi' => 'Gia đình có trẻ em', 'en' => 'Family with kids'),
        'cap-doi-lang-man' => array('vi' => 'Cặp đôi & lãng mạn', 'en' => 'Couples & romance'),
        'festival-hoa' => array('vi' => 'Mùa hoa & festival', 'en' => 'Flower seasons & festival'),
        'am-thuc-cao-nguyen' => array('vi' => 'Ẩm thực cao nguyên', 'en' => 'Highland food'),
    ),

    'review_platforms' => array(
        array('code' => 'tripadvisor', 'name' => 'Tripadvisor', 'rating' => 4.8, 'review_count' => 312, 'sort' => 0,
            'quote' => 'Được khách quốc tế đánh giá cao về tour cà phê Cầu Đất và canyoning Datanla.',
            'link_label' => 'Đọc đánh giá trên Tripadvisor', 'url' => 'https://www.tripadvisor.com'),
        array('code' => 'google', 'name' => 'Google', 'rating' => 4.7, 'review_count' => 480, 'sort' => 1,
            'quote' => '4.7/5 trên Google Maps — khách Sài Gòn khen limousine cuối tuần và tư vấn homestay theo khu.',
            'link_label' => 'Xem đánh giá trên Google', 'url' => 'https://www.google.com/maps'),
        array('code' => 'trustpilot', 'name' => 'Trustpilot', 'rating' => 4.6, 'review_count' => 86, 'sort' => 2,
            'quote' => 'Điểm "Xuất sắc" — đặc biệt tour trăng mật và hỗ trợ đổi lịch khi sương mù dày.',
            'link_label' => 'Đọc đánh giá trên Trustpilot', 'url' => 'https://www.trustpilot.com'),
    ),

    'cruise_types' => array(
        array('slug' => 'thuyen-ho-tuyen-lam', 'name' => 'Thuyền hồ Tuyền Lâm', 'count' => 2, 'image' => null, 'imageHero' => null, 'imageSrcset' => null, 'sort' => 10),
        array('slug' => 'sup-kayak-tuyen-lam', 'name' => 'SUP & kayak Tuyền Lâm', 'count' => 1, 'image' => null, 'imageHero' => null, 'imageSrcset' => null, 'sort' => 20),
    ),

    'home_slides' => array(
        array(
            'sort' => 0, 'text_align' => 'center', 'link_url' => '/tours',
            'vi' => array('title' => 'Đà Lạt', 'title_accent' => 'thành phố ngàn hoa trên cao nguyên',
                'description' => 'Nhiệt độ dịu mát quanh năm, hồ Xuân Hương, thung lũng hoa và những đồi chè — không gian lý tưởng để nghỉ ngơi và khám phá.',
                'button_label' => 'Khám phá Đà Lạt', 'image_alt' => 'Đà Lạt — thành phố ngàn hoa trên cao nguyên Lâm Đồng'),




'en' => array('title' => 'Da Lat', 'title_accent' => 'city of a thousand flowers on the plateau',
                'description' => 'Mild temperatures year-round, Xuan Huong Lake, flower valleys and tea hills — an ideal space to rest and explore.',
                'button_label' => 'Discover Da Lat', 'image_alt' => 'Da Lat — flower city on the Lam Dong plateau'),
        ),
        array(
            'sort' => 1, 'text_align' => 'center', 'link_url' => '/diem-den/cau-dat',
            'vi' => array('title' => 'Cầu Đất & đồi chè', 'title_accent' => 'cà phê specialty & sương sớm',
                'description' => 'Cao nguyên trên 1.600m — tour nông trại cà phê, chụp ảnh đồi chè và thưởng thức ly Arabica ngay tại vườn.',
                'button_label' => 'Xem tour Cầu Đất', 'image_alt' => 'Đồi chè và cà phê Cầu Đất Đà Lạt'),




'en' => array('title' => 'Cau Dat & tea hills', 'title_accent' => 'specialty coffee & morning mist',
                'description' => 'Plateau above 1,600m — coffee farm tours, tea-hill photography and Arabica tastings right in the garden.',
                'button_label' => 'View Cau Dat tours', 'image_alt' => 'Cau Dat tea hills and coffee Da Lat'),
        ),
        array(
            'sort' => 2, 'text_align' => 'center', 'link_url' => '/diem-den/langbiang-ta-nung',
            'vi' => array('title' => 'Langbiang & phiêu lưu', 'title_accent' => 'trekking, jeep & canyoning',
                'description' => 'Chinh phục đỉnh Langbiang, trượt máng Datanla hay canyoning thác — trải nghiệm mạo hiểm an toàn với đội ngũ địa phương.',
                'button_label' => 'Xem tour phiêu lưu', 'image_alt' => 'Trekking Langbiang và canyoning Đà Lạt'),




'en' => array('title' => 'Langbiang & adventure', 'title_accent' => 'trekking, jeep & canyoning',
                'description' => 'Summit Langbiang, ride the Datanla coaster or canyon the falls — safe adventure with local guides.',
                'button_label' => 'View adventure tours', 'image_alt' => 'Langbiang trekking and canyoning Da Lat'),
        ),
    ),

    'zones' => array(
        array('slug' => 'trung-tam-xuan-huong', 'name' => 'Trung tâm & Hồ Xuân Hương', 'size' => 'large', 'tourCount' => 6, 'tagline' => 'Nhà thờ Con Gà, chợ đêm, Crazy House và nhịp sống phố núi'),
        array('slug' => 'langbiang-ta-nung', 'name' => 'Langbiang & Tà Nung', 'size' => 'large', 'tourCount' => 4, 'tagline' => 'Đỉnh Langbiang, làng dân tộc và đường rừng thông'),
        array('slug' => 'tuyen-lam-truc-lam', 'name' => 'Tuyền Lâm & Trúc Lâm', 'size' => 'normal', 'tourCount' => 3, 'tagline' => 'Hồ núi lớn nhất, thiền viện và thuyền trên hồ'),
        array('slug' => 'datanla-robin-hill', 'name' => 'Datanla & Robin Hill', 'size' => 'normal', 'tourCount' => 3, 'tagline' => 'Thác Datanla, máng trượt và cáp treo Robin Hill'),
        array('slug' => 'cau-dat', 'name' => 'Cao nguyên Cầu Đất', 'size' => 'normal', 'tourCount' => 3, 'tagline' => 'Cà phê specialty, đồi chè và khí hậu mát quanh năm'),
        array('slug' => 'ngoai-o-thanh-pho', 'name' => 'Ngoại ô & thác nước', 'size' => 'normal', 'tourCount' => 3, 'tagline' => 'Thác Voi, Đường hầm đất sét, làng Cù Lần'),
        array('slug' => 'ket-hop-nha-trang', 'name' => 'Kết hợp Nha Trang', 'size' => 'normal', 'tourCount' => 2, 'tagline' => 'Nối cao nguyên mát và biển Nha Trang trong một hành trình'),
        array('slug' => 'ket-hop-mui-ne', 'name' => 'Kết hợp Mũi Né', 'size' => 'normal', 'tourCount' => 2, 'tagline' => 'Đà Lạt — đồi cát Phan Thiết: hai khí hậu trong một chuyến'),
    ),

    'zone_translations' => array(
        'trung-tam-xuan-huong' => array('vi' => 'Trung tâm & Hồ Xuân Hương',



'en' => 'City centre & Xuan Huong Lake',
            'tagline' => array('vi' => 'Nhà thờ Con Gà, chợ đêm, Crazy House và nhịp sống phố núi',



'en' => 'Chicken Church, night market, Crazy House and mountain-town life')),
        'langbiang-ta-nung' => array('vi' => 'Langbiang & Tà Nung',



'en' => 'Langbiang & Ta Nung',
            'tagline' => array('vi' => 'Đỉnh Langbiang, làng dân tộc và đường rừng thông',



'en' => 'Langbiang summit, ethnic villages and pine forest trails')),
        'tuyen-lam-truc-lam' => array('vi' => 'Tuyền Lâm & Trúc Lâm',



'en' => 'Tuyen Lam & Truc Lam',
            'tagline' => array('vi' => 'Hồ núi lớn nhất, thiền viện và thuyền trên hồ',



'en' => 'Largest mountain lake, zen monastery and lake boats')),
        'datanla-robin-hill' => array('vi' => 'Datanla & Robin Hill',



'en' => 'Datanla & Robin Hill',
            'tagline' => array('vi' => 'Thác Datanla, máng trượt và cáp treo Robin Hill',



'en' => 'Datanla Falls, alpine coaster and Robin Hill cable car')),
        'cau-dat' => array('vi' => 'Cao nguyên Cầu Đất',



'en' => 'Cau Dat highlands',
            'tagline' => array('vi' => 'Cà phê specialty, đồi chè và khí hậu mát quanh năm',



'en' => 'Specialty coffee, tea hills and cool climate year-round')),
        'ngoai-o-thanh-pho' => array('vi' => 'Ngoại ô & thác nước',



'en' => 'Outskirts & waterfalls',
            'tagline' => array('vi' => 'Thác Voi, Đường hầm đất sét, làng Cù Lần',



'en' => 'Elephant Falls, Clay Tunnel, Cu Lan village')),
        'ket-hop-nha-trang' => array('vi' => 'Kết hợp Nha Trang',



'en' => 'Combined with Nha Trang',
            'tagline' => array('vi' => 'Nối cao nguyên mát và biển Nha Trang trong một hành trình',



'en' => 'Cool highlands and Nha Trang beach in one journey')),
        'ket-hop-mui-ne' => array('vi' => 'Kết hợp Mũi Né',



'en' => 'Combined with Mui Ne',
            'tagline' => array('vi' => 'Đà Lạt — đồi cát Phan Thiết: hai khí hậu trong một chuyến',



'en' => 'Da Lat highlands and Mui Ne dunes — two climates in one trip')),
    ),

    'tours' => array(
        array(
            'slug' => 'da-lat-3-ngay-tong-quan',
            'title' => 'Đà Lạt 3 ngày 2 đêm — Tổng quan thành phố ngàn hoa',
            'zoneSlug' => 'trung-tam-xuan-huong',
            'zone' => 'Trung tâm & Hồ Xuân Hương',
            'tourCode' => 'DL3D-01',
            'duration' => '3 ngày 2 đêm',
            'days' => 3,
            'rating' => 4.9,
            'reviewCount' => 186,
            'badge' => 'Bán chạy nhất',
            'featured' => true,
            'styles' => array(
                '3n2d',
                'cafe-nong-trai',
                'cuoi-tuan-sai-gon',
            ),
            'quote' => array(
                'text' => 'Ba ngày vừa đủ để cảm nhận Đà Lạt — từ hồ Xuân Hương, thác Datanla đến đồi chè Cầu Đất.',
                'author' => 'Chị Minh Anh',
            ),
            'places' => array(
                'Hồ Xuân Hương',
                'Crazy House',
                'Thác Datanla',
                'Cầu Đất',
                'Chợ đêm',
            ),
            'start' => 'Đà Lạt',
            'end' => 'Đà Lạt',
            'highlightsIntro' => 'Lịch trình cân bằng cho lần đầu: city tour, thác nước, cà phê và buổi tối chợ đêm.',
            'highlights' => array(
                'City tour các điểm biểu tượng',
                'Thác Datanla + máng trượt',
                'Tour cà phê Cầu Đất',
                'Tour ẩm thực chợ đêm',
                'Tự do khám phá phố đi bộ',
            ),
            'itinerary' => array(
                array(
                    'day' => 1,
                    'title' => 'Đón — City tour & hồ Xuân Hương',
                    'meals' => 'Trưa; Tối',
                    'transport' => array(
                        'car',
                    ),
                    'overnight' => 'Trung tâm Đà Lạt',
                    'content' => 'Đón sân bay/bến xe, city tour Crazy House, Nhà thờ Con Gà, dạo hồ Xuân Hương. Tối chợ đêm Đà Lạt.',
                ),
                array(
                    'day' => 2,
                    'title' => 'Datanla — Cầu Đất',
                    'meals' => 'Sáng; Trưa; Tối',
                    'transport' => array(
                        'car',
                    ),
                    'overnight' => 'Trung tâm Đà Lạt',
                    'content' => 'Sáng thác Datanla và máng trượt. Chiều tour nông trại cà phê Cầu Đát, thưởng thức cupping.',
                ),
                array(
                    'day' => 3,
                    'title' => 'Tuyền Lâm — Trúc Lâm — Tiễn',
                    'meals' => 'Sáng; Trưa',
                    'transport' => array(
                        'car',
                        'cable_car',
                    ),
                    'overnight' => null,
                    'content' => 'Cáp treo Robin Hill, thiền viện Trúc Lâm, hồ Tuyền Lâm. Ăn trưa và tiễn sân bay/bến xe.',
                ),
            ),
            'inclusions' => array(
                'Xe đưa đón trong chương trình',
                'Vé tham quan theo lịch',
                'HDV địa phương',
                'Bữa ăn ghi trong chương trình',
            ),
            'exclusions' => array(
                'Vé máy bay/xe liên tỉnh (đặt thêm)',
                'Lưu trú (đặt thêm)',
                'Đồ uống',
                'Tip',
            ),
            'notes' => array(
                'Có thể gộp đặt homestay, villa hoặc resort qua :brand theo khu vực bạn chọn.',
                'Lịch có thể đổi theo thời tiết sương mù.',
            ),
            'faqs' => array(
                array(
                    'q' => 'Tour có bao gồm khách sạn không?',
                    'a' => 'Giá tour chưa gồm phòng nghỉ. :brand hỗ trợ gợi ý và đặt homestay, villa hoặc resort theo ngân sách và khu vực — ghép vào cùng chuyến đi.',
                ),
                array(
                    'q' => 'Đà Lạt mát không, cần áo ấm không?',
                    'a' => 'Nhiệt độ thường 15–24°C, tối có thể xuống 10–12°C mùa khô — nên mang áo khoác mỏng.',
                ),
            ),
            'galleryCount' => 6,
            'priceFrom' => 2850000,
            'currency' => 'VND',
        ),
        array(
            'slug' => 'da-lat-2-ngay-lang-man',
            'title' => 'Đà Lạt 2 ngày 1 đêm — Lãng mạn & hoa',
            'zoneSlug' => 'trung-tam-xuan-huong',
            'zone' => 'Trung tâm & Hồ Xuân Hương',
            'tourCode' => 'DL2D-01',
            'duration' => '2 ngày 1 đêm',
            'days' => 2,
            'rating' => 4.9,
            'reviewCount' => 124,
            'badge' => 'Cặp đôi yêu thích',
            'featured' => true,
            'styles' => array(
                '2n1d',
                'cap-doi-lang-man',
                'cuoi-tuan-sai-gon',
            ),
            'quote' => array(
                'text' => 'Bó hoa surprise ở vườn hoa và hoàng hôn hồ Tuyền Lâm — đúng kiểu trăng mật mini.',
                'author' => 'Vợ chồng Tuấn — Hà',
            ),
            'places' => array(
                'Vườn hoa thành phố',
                'Hồ Tuyền Lâm',
                'Lumiere',
                'Hồ Xuân Hương',
            ),
            'start' => 'Đà Lạt',
            'end' => 'Đà Lạt',
            'highlightsIntro' => 'Dành cho cặp đôi: vườn hoa, chụp ảnh Lumiere, thuyền hoàng hôn Tuyền Lâm.',
            'highlights' => array(
                'Vườn hoa & setup chụp ảnh',
                'Lumiere Light Garden buổi tối',
                'Thuyền picnic hoàng hôn Tuyền Lâm',
                'Bữa tối lãng mạn gợi ý',
            ),
            'itinerary' => array(
                array(
                    'day' => 1,
                    'title' => 'Hoa & ánh sáng',
                    'meals' => 'Trưa; Tối',
                    'transport' => array(
                        'car',
                    ),
                    'overnight' => 'Trung tâm Đà Lạt',
                    'content' => 'Vườn hoa, chụp ảnh couple, chiều Lumiere, tối tour ẩm thực hoặc nhà hàng view hồ.',
                ),
                array(
                    'day' => 2,
                    'title' => 'Tuyền Lâm buổi sáng — Tiễn',
                    'meals' => 'Sáng',
                    'transport' => array(
                        'car',
                        'boat',
                    ),
                    'overnight' => null,
                    'content' => 'Sáng thuyền Tuyền Lâm, thiền viện Trúc Lâm, tiễn khách.',
                ),
            ),
            'inclusions' => array(
                'Xe riêng 2 ngày',
                'Vé vườn hoa, Lumiere',
                'Thuyền Tuyền Lâm',
                'HDV chụp ảnh cơ bản',
            ),
            'exclusions' => array(
                'Lưu trú',
                'Bữa tối fine-dining',
                'Trang phục chụp ảnh',
            ),
            'notes' => array(
                'Có thể thêm dịch vụ hoa surprise qua mục Dịch vụ.',
            ),
            'faqs' => array(),
            'galleryCount' => 5,
            'priceFrom' => 1950000,
            'currency' => 'VND',
        ),
        array(
            'slug' => 'da-lat-4-ngay-kham-pha-sau',
            'title' => 'Đà Lạt 4 ngày 3 đêm — Khám phá sâu cao nguyên',
            'zoneSlug' => 'langbiang-ta-nung',
            'zone' => 'Langbiang & Tà Nung',
            'tourCode' => 'DL4D-01',
            'duration' => '4 ngày 3 đêm',
            'days' => 4,
            'rating' => 4.8,
            'reviewCount' => 67,
            'badge' => null,
            'featured' => true,
            'styles' => array(
                '4n3d',
                'phieu-luu-canyon',
                'cafe-nong-trai',
            ),
            'quote' => array(
                'text' => 'Bốn ngày mới đủ để leo Langbiang, canyoning và ngủ một đêm gần Cầu Đất.',
                'author' => 'Anh Quốc Bảo',
            ),
            'places' => array(
                'Langbiang',
                'Datanla',
                'Cầu Đất',
                'Cù Lần',
                'Thác Voi',
            ),
            'start' => 'Đà Lạt',
            'end' => 'Đà Lạt',
            'highlightsIntro' => 'Hành trình đầy đủ: trekking, canyoning, nông trại và ngoại ô thác Voi.',
            'highlights' => array(
                'Trekking / jeep Langbiang',
                'Canyoning Datanla',
                '2 ngày Cầu Đất cà phê & chè',
                'Làng Cù Lần & Thác Voi',
                'City tour ngắn ngày cuối',
            ),
            'itinerary' => array(
                array(
                    'day' => 1,
                    'title' => 'City & Datanla',
                    'meals' => 'Trưa; Tối',
                    'transport' => array(
                        'car',
                    ),
                    'overnight' => 'Trung tâm',
                    'content' => 'City tour, chiều Datanla coaster.',
                ),
                array(
                    'day' => 2,
                    'title' => 'Langbiang & canyoning',
                    'meals' => 'Sáng; Trưa; Tối',
                    'transport' => array(
                        'car',
                        'jeep',
                    ),
                    'overnight' => 'Trung tâm',
                    'content' => 'Sáng jeep Langbiang, chiều canyoning (tuỳ điều kiện thời tiết).',
                ),
                array(
                    'day' => 3,
                    'title' => 'Cầu Đất — Cù Lần',
                    'meals' => 'Sáng; Trưa; Tối',
                    'transport' => array(
                        'car',
                    ),
                    'overnight' => 'Cầu Đát / trung tâm',
                    'content' => 'Tour cà phê, chiều làng Cù Lần xe lửa mini.',
                ),
                array(
                    'day' => 4,
                    'title' => 'Thác Voi — Tiễn',
                    'meals' => 'Sáng; Trưa',
                    'transport' => array(
                        'car',
                    ),
                    'overnight' => null,
                    'content' => 'Thác Voi, Đường hầm đất sét, tiễn khách.',
                ),
            ),
            'inclusions' => array(
                'Xe riêng',
                'Vé & hoạt động theo lịch',
                'HDV',
                'Canyoning có HLV',
            ),
            'exclusions' => array(
                'Lưu trú',
                'Xe liên tỉnh',
                'Bảo hiểm thể thao canyoning',
            ),
            'notes' => array(
                'Canyoning có thể thay bằng ATV/jeep khi mưa lớn.',
            ),
            'faqs' => array(),
            'galleryCount' => 6,
            'priceFrom' => 4200000,
            'currency' => 'VND',
        ),
        array(
            'slug' => 'city-tour-da-lat-1-ngay',
            'title' => 'Tour thành phố Đà Lạt 1 ngày — Biểu tượng & linh thiền',
            'zoneSlug' => 'trung-tam-xuan-huong',
            'zone' => 'Trung tâm & Hồ Xuân Hương',
            'tourCode' => 'DL1D-01',
            'duration' => '1 ngày',
            'days' => 1,
            'rating' => 4.8,
            'reviewCount' => 312,
            'badge' => 'Phổ biến',
            'featured' => true,
            'styles' => array(
                'day-trip',
                'gia-dinh-tre-em',
            ),
            'quote' => array(
                'text' => 'Một ngày gom hết Crazy House, Linh Phước và hồ — không cần tự lái xe.',
                'author' => 'Gia đình chị Thảo',
            ),
            'places' => array(
                'Crazy House',
                'Linh Phước',
                'Domaine de Marie',
                'Hồ Xuân Hương',
            ),
            'start' => 'Đà Lạt',
            'end' => 'Đà Lạt',
            'highlightsIntro' => 'Tour ghép hoặc riêng — các điểm check-in kinh điển trong một ngày.',
            'highlights' => array(
                'Crazy House (Hang Nga)',
                'Chùa Linh Phước ve chai',
                'Domaine de Marie',
                'Dạo hồ Xuân Hương',
            ),
            'itinerary' => array(
                array(
                    'day' => 1,
                    'title' => 'Tour thành phố trọn ngày',
                    'meals' => 'Trưa',
                    'transport' => array(
                        'car',
                    ),
                    'overnight' => null,
                    'content' => '08:30–17:00 tham quan các điểm, ăn trưa quán địa phương, chiều tự do chợ Đà Lạt.',
                ),
            ),
            'inclusions' => array(
                'Xe 16 chỗ hoặc riêng',
                'Vé Crazy House, Linh Phước',
                'HDV',
                'Bữa trưa',
            ),
            'exclusions' => array(
                'Đồ uống',
                'Mua sắm chợ',
            ),
            'notes' => array(
                'Ghép nhóm max 12 khách.',
            ),
            'faqs' => array(),
            'galleryCount' => 4,
            'priceFrom' => 650000,
            'currency' => 'VND',
        ),
        array(
            'slug' => 'canyoning-datanla-1-ngay',
            'title' => 'Canyoning thác Datanla — 1 ngày phiêu lưu',
            'zoneSlug' => 'datanla-robin-hill',
            'zone' => 'Datanla & Robin Hill',
            'tourCode' => 'DL1D-02',
            'duration' => '1 ngày',
            'days' => 1,
            'rating' => 4.9,
            'reviewCount' => 198,
            'badge' => 'Mạo hiểm',
            'featured' => true,
            'styles' => array(
                'day-trip',
                'phieu-luu-canyon',
            ),
            'quote' => array(
                'text' => 'Nhảy thác 15m — sợ ban đầu nhưng HLV cực kỳ chuyên nghiệp.',
                'author' => 'Bạn Minh Khoa',
            ),
            'places' => array(
                'Thác Datanla',
                'Rừng thông Datanla',
            ),
            'start' => 'Đà Lạt',
            'end' => 'Đà Lạt',
            'highlightsIntro' => 'Hoạt động đặc trưng của Đà Lạt — leo thác, zipline, nhảy nước với HLV chứng nhận.',
            'highlights' => array(
                'HLV quốc tế',
                'Thiết bị đầy đủ',
                'Nhảy thác an toàn',
                'Nhóm 4–8 khách',
            ),
            'itinerary' => array(
                array(
                    'day' => 1,
                    'title' => 'Canyoning Datanla',
                    'meals' => 'Trưa',
                    'transport' => array(
                        'car',
                    ),
                    'overnight' => null,
                    'content' => 'Sáng briefing, 3–4 giờ canyoning, trưa tại khu vực thác, về trước 16h.',
                ),
            ),
            'inclusions' => array(
                'HLV',
                'Thiết bị',
                'Bảo hiểm cơ bản',
                'Trưa',
            ),
            'exclusions' => array(
                'Đồ cá nhân ướt',
                'Video GoPro',
            ),
            'notes' => array(
                'Biết bơi; từ 16 tuổi; hủy khi mưa lớn.',
            ),
            'faqs' => array(),
            'galleryCount' => 4,
            'priceFrom' => 1200000,
            'currency' => 'VND',
        ),
        array(
            'slug' => 'langbiang-jeep-trekking-1-ngay',
            'title' => 'Langbiang — Jeep & trekking đỉnh 1 ngày',
            'zoneSlug' => 'langbiang-ta-nung',
            'zone' => 'Langbiang & Tà Nung',
            'tourCode' => 'DL1D-03',
            'duration' => '1 ngày',
            'days' => 1,
            'rating' => 4.8,
            'reviewCount' => 156,
            'badge' => null,
            'featured' => true,
            'styles' => array(
                'day-trip',
                'phieu-luu-canyon',
            ),
            'quote' => array(
                'text' => 'View đỉnh Langbiang nhìn toàn thành phố trong sương — đáng mọi bước chân.',
                'author' => 'Anh Đức Thắng',
            ),
            'places' => array(
                'Núi Langbiang',
                'Làng K\'Ho',
            ),
            'start' => 'Đà Lạt',
            'end' => 'Đà Lạt',
            'highlightsIntro' => 'Jeep lên đồi hoặc trekking chân đến đỉnh 2.167m — ngọn cao nhất gần thành phố.',
            'highlights' => array(
                'Jeep địa hình hoặc trek',
                'View toàn cảnh Đà Lạt',
                'Gặp gỡ văn hoá K\'Ho',
                'Trưa nhà dân / picnic',
            ),
            'itinerary' => array(
                array(
                    'day' => 1,
                    'title' => 'Langbiang summit',
                    'meals' => 'Trưa',
                    'transport' => array(
                        'car',
                        'jeep',
                        'trekking',
                    ),
                    'overnight' => null,
                    'content' => 'Sáng xe tới chân núi, jeep hoặc trek lên đỉnh, trưa dã ngoại, chiều về.',
                ),
            ),
            'inclusions' => array(
                'Xe',
                'Jeep / HDV trek',
                'Vé khu du lịch',
                'Trưa',
            ),
            'exclusions' => array(
                'Tip',
                'Áo ấm (nên tự mang)',
            ),
            'notes' => array(
                'Sương mù buổi sáng — view phụ thuộc thời tiết.',
            ),
            'faqs' => array(),
            'galleryCount' => 4,
            'priceFrom' => 850000,
            'currency' => 'VND',
        ),
        array(
            'slug' => 'cau-dat-cafe-tea-1-ngay',
            'title' => 'Cầu Đất — Tour cà phê & đồi chè 1 ngày',
            'zoneSlug' => 'cau-dat',
            'zone' => 'Cao nguyên Cầu Đất',
            'tourCode' => 'DL1D-04',
            'duration' => '1 ngày',
            'days' => 1,
            'rating' => 4.9,
            'reviewCount' => 203,
            'badge' => 'Cà phê',
            'featured' => true,
            'styles' => array(
                'day-trip',
                'cafe-nong-trai',
            ),
            'quote' => array(
                'text' => 'Hiểu luôn sự khác biệt Arabica Cầu Đất sau buổi cupping — không uống cà phê như trước nữa.',
                'author' => 'Chị Lan Coffee',
            ),
            'places' => array(
                'Cầu Đất Farm',
                'Đồi chè',
                'Dalat Milk',
            ),
            'start' => 'Đà Lạt',
            'end' => 'Đà Lạt',
            'highlightsIntro' => 'Du lịch nông nghiệp đặc trưng: thu hoạch, chế biến, cupping và chụp đồi chê.',
            'highlights' => array(
                'Tham quan vườn cà phê',
                'Cupping specialty',
                'Đồi chè & dâu tây',
                'Ghé Dalat Milk (tuỳ chọn)',
            ),
            'itinerary' => array(
                array(
                    'day' => 1,
                    'title' => 'Cầu Đất farm tour',
                    'meals' => 'Trưa',
                    'transport' => array(
                        'car',
                    ),
                    'overnight' => null,
                    'content' => 'Sáng farm tour, cupping, trưa farm-to-table, chiều đồi chè chụp ảnh.',
                ),
            ),
            'inclusions' => array(
                'Xe',
                'HDV cà phê',
                'Cupping',
                'Trưa',
            ),
            'exclusions' => array(
                'Mua cà phê mang về',
            ),
            'notes' => array(
                'Mùa thu hoạch chính tháng 11–1.',
            ),
            'faqs' => array(),
            'galleryCount' => 5,
            'priceFrom' => 750000,
            'currency' => 'VND',
        ),
        array(
            'slug' => 'thac-voi-cu-lan-1-ngay',
            'title' => 'Thác Voi — Đường hầm đất sét — Cù Lần 1 ngày',
            'zoneSlug' => 'ngoai-o-thanh-pho',
            'zone' => 'Ngoại ô & thác nước',
            'tourCode' => 'DL1D-05',
            'duration' => '1 ngày',
            'days' => 1,
            'rating' => 4.7,
            'reviewCount' => 134,
            'badge' => null,
            'featured' => false,
            'styles' => array(
                'day-trip',
                'gia-dinh-tre-em',
            ),
            'quote' => array(
                'text' => 'Con thích xe lửa Cù Lần, bố mẹ thích Thác Voi — ai cũng có kỷ niệm.',
                'author' => 'Gia đình anh Việt',
            ),
            'places' => array(
                'Thác Voi',
                'Đường hầm đất sét',
                'Làng Cù Lần',
            ),
            'start' => 'Đà Lạt',
            'end' => 'Đà Lạt',
            'highlightsIntro' => 'Tour ngoại ô phù hợp gia đình — thác 30m, hầm điêu khác đất sét và làng cổ tích.',
            'highlights' => array(
                'Thác Voi (Elephant Falls)',
                'Clay Tunnel diorama',
                'Cù Lần xe lửa mini',
                'Buffet dâu / sữa chua',
            ),
            'itinerary' => array(
                array(
                    'day' => 1,
                    'title' => 'Ngoại ô Đà Lạt',
                    'meals' => 'Trưa',
                    'transport' => array(
                        'car',
                    ),
                    'overnight' => null,
                    'content' => 'Thác Voi sáng, Clay Tunnel trưa, Cù Lần chiều.',
                ),
            ),
            'inclusions' => array(
                'Xe',
                'Vé các điểm',
                'HDV',
                'Trưa',
            ),
            'exclusions' => array(
                'Hoạt động mạo hiểm Cù Lần',
            ),
            'notes' => array(
                'Đường đèo — say xe nên uống thuốc trước.',
            ),
            'faqs' => array(),
            'galleryCount' => 4,
            'priceFrom' => 720000,
            'currency' => 'VND',
        ),
        array(
            'slug' => 'festival-hoa-da-lat-2n1d',
            'title' => 'Festival hoa Đà Lạt — 2 ngày 1 đêm (cuối tháng 12)',
            'zoneSlug' => 'trung-tam-xuan-huong',
            'zone' => 'Trung tâm & Hồ Xuân Hương',
            'tourCode' => 'DL2D-02',
            'duration' => '2 ngày 1 đêm',
            'days' => 2,
            'rating' => 4.9,
            'reviewCount' => 89,
            'badge' => 'Mùa hoa',
            'featured' => true,
            'styles' => array(
                '2n1d',
                'festival-hoa',
                'cuoi-tuan-sai-gon',
            ),
            'quote' => array(
                'text' => 'Đúng mùa festival — đường phố ngập hoa, không khí như lễ hội thật sự.',
                'author' => 'Chị Hồng Hoa',
            ),
            'places' => array(
                'Đường hoa',
                'Vườn hoa',
                'Hồ Xuân Hương',
            ),
            'start' => 'Đà Lạt',
            'end' => 'Đà Lạt',
            'highlightsIntro' => 'Gói riêng mùa Festival hoa (thường 20–31/12): đường hoa, parade và chụp ảnh.',
            'highlights' => array(
                'Tham quan đường hoa',
                'Vườn hoa festival',
                'Photo walk có HDV',
                'Gợi ý homestay view hoa',
            ),
            'itinerary' => array(
                array(
                    'day' => 1,
                    'title' => 'Khai mạc & đường hoa',
                    'meals' => 'Trưa; Tối',
                    'transport' => array(
                        'car',
                    ),
                    'overnight' => 'Trung tâm',
                    'content' => 'Chiều tối dạo đường hoa, chụp ảnh.',
                ),
                array(
                    'day' => 2,
                    'title' => 'Vườn hoa — Tiễn',
                    'meals' => 'Sáng',
                    'transport' => array(
                        'car',
                    ),
                    'overnight' => null,
                    'content' => 'Sáng vườn hoa, tiễn khách.',
                ),
            ),
            'inclusions' => array(
                'Xe',
                'HDV',
                'Vé vườn hoa',
            ),
            'exclusions' => array(
                'Lưu trú (nên đặt sớm mùa festival)',
                'Vé xe vào Đà Lạt mùa festival',
            ),
            'notes' => array(
                'Chỉ vận hành khoảng cuối tháng 12 — đặt trước 2–4 tuần.',
            ),
            'faqs' => array(),
            'galleryCount' => 5,
            'priceFrom' => 1650000,
            'currency' => 'VND',
        ),
        array(
            'slug' => 'gia-dinh-da-lat-2n1d',
            'title' => 'Đà Lạt gia đình 2 ngày 1 đêm — Nhẹ nhàng & vui',
            'zoneSlug' => 'trung-tam-xuan-huong',
            'zone' => 'Trung tâm & Hồ Xuân Hương',
            'tourCode' => 'DL2D-03',
            'duration' => '2 ngày 1 đêm',
            'days' => 2,
            'rating' => 4.8,
            'reviewCount' => 145,
            'badge' => 'Gia đình',
            'featured' => true,
            'styles' => array(
                '2n1d',
                'gia-dinh-tre-em',
                'cuoi-tuan-sai-gon',
            ),
            'quote' => array(
                'text' => 'Bé 5 tuổi mê Zoodoo và dâu tây — bố mẹ được nghỉ thật sự.',
                'author' => 'Chị Ngọc Ánh',
            ),
            'places' => array(
                'Zoodoo',
                'Vườn dâu',
                'Datanla coaster',
                'Chợ đêm',
            ),
            'start' => 'Đà Lạt',
            'end' => 'Đà Lạt',
            'highlightsIntro' => 'Lịch nhẹ cho trẻ nhỏ: thú mini, hái dâu, máng trượt Datanla.',
            'highlights' => array(
                'Zoodoo petting zoo',
                'Hái dâu tây',
                'Máng trượt Datanla (an toàn)',
                'Chợ đêm ăn vặt',
            ),
            'itinerary' => array(
                array(
                    'day' => 1,
                    'title' => 'Zoodoo & dâu',
                    'meals' => 'Trưa; Tối',
                    'transport' => array(
                        'car',
                    ),
                    'overnight' => 'Trung tâm',
                    'content' => 'Sáng Zoodoo, chiều vườn dâu.',
                ),
                array(
                    'day' => 2,
                    'title' => 'Datanla — Tiễn',
                    'meals' => 'Sáng; Trưa',
                    'transport' => array(
                        'car',
                    ),
                    'overnight' => null,
                    'content' => 'Datanla coaster, tiễn.',
                ),
            ),
            'inclusions' => array(
                'Xe riêng',
                'Vé điểm tham quan',
                'HDV gia đình',
            ),
            'exclusions' => array(
                'Lưu trú',
                'Ăn tối chợ',
            ),
            'notes' => array(),
            'faqs' => array(),
            'galleryCount' => 4,
            'priceFrom' => 1750000,
            'currency' => 'VND',
        ),
        array(
            'slug' => 'da-lat-nha-trang-3n2d',
            'title' => 'Đà Lạt — Nha Trang 3 ngày 2 đêm',
            'zoneSlug' => 'ket-hop-nha-trang',
            'zone' => 'Kết hợp Nha Trang',
            'tourCode' => 'DLNT3D-01',
            'duration' => '3 ngày 2 đêm',
            'days' => 3,
            'rating' => 4.8,
            'reviewCount' => 76,
            'badge' => 'Combo',
            'featured' => true,
            'styles' => array(
                '3n2d',
                'cafe-nong-trai',
            ),
            'quote' => array(
                'text' => 'Mát ở Đà Lạt, tắm biển Nha Trang — combo hoàn hảo từ Sài Gòn.',
                'author' => 'Vợ chồng anh Phong',
            ),
            'places' => array(
                'Đà Lạt',
                'Đèo Khánh Vĩnh',
                'Nha Trang',
            ),
            'start' => 'Đà Lạt',
            'end' => 'Nha Trang',
            'highlightsIntro' => '2 đêm Đà Lạt + 1 đêm Nha Trang (hoặc ngược) — xe limousine/bus nối sẵn.',
            'highlights' => array(
                'City tour Đà Lạt',
                'Cà phê Cầu Đất',
                'Xe xuống Nha Trang ~3.5h',
                'Gợi ý beach time',
            ),
            'itinerary' => array(
                array(
                    'day' => 1,
                    'title' => 'Đà Lạt city',
                    'meals' => 'Trưa; Tối',
                    'transport' => array(
                        'car',
                    ),
                    'overnight' => 'Đà Lạt',
                    'content' => 'City tour, chợ đêm.',
                ),
                array(
                    'day' => 2,
                    'title' => 'Cầu Đất — xuống Nha Trang',
                    'meals' => 'Sáng; Tối',
                    'transport' => array(
                        'car',
                        'bus',
                    ),
                    'overnight' => 'Nha Trang',
                    'content' => 'Sáng Cầu Đất, chiều xe xuống biển.',
                ),
                array(
                    'day' => 3,
                    'title' => 'Nha Trang tự do — Kết thúc',
                    'meals' => 'Sáng',
                    'transport' => array(
                        'car',
                    ),
                    'overnight' => null,
                    'content' => 'Tự do biển, tiễn sân bay/bến.',
                ),
            ),
            'inclusions' => array(
                'Xe Đà Lạt',
                'Vé tham quan',
                'Xe Đà Lạt—Nha Trang',
                'HDV Đà Lạt',
            ),
            'exclusions' => array(
                'Lưu trú',
                'Tour biển Nha Trang',
                'Vé bay về',
            ),
            'notes' => array(
                'Có thể đảo chiều bắt đầu từ Nha Trang.',
            ),
            'faqs' => array(),
            'galleryCount' => 5,
            'priceFrom' => 3200000,
            'currency' => 'VND',
        ),
        array(
            'slug' => 'da-lat-mui-ne-3n2d',
            'title' => 'Đà Lạt — Mũi Né 3 ngày 2 đêm',
            'zoneSlug' => 'ket-hop-mui-ne',
            'zone' => 'Kết hợp Mũi Né',
            'tourCode' => 'DLMN3D-01',
            'duration' => '3 ngày 2 đêm',
            'days' => 3,
            'rating' => 4.7,
            'reviewCount' => 54,
            'badge' => null,
            'featured' => false,
            'styles' => array(
                '3n2d',
                'cuoi-tuan-sai-gon',
            ),
            'quote' => array(
                'text' => 'Sương Đà Lạt buổi sáng, đồi cát Mũi Né chiều — contrast tuyệt vời.',
                'author' => 'Nhóm bạn Sài Gòn',
            ),
            'places' => array(
                'Đà Lạt',
                'Đồi cát',
                'Mũi Né',
            ),
            'start' => 'Đà Lạt',
            'end' => 'Mũi Né',
            'highlightsIntro' => 'Cao nguyên + sa mạc mini — phổ biến với khách miền Nam.',
            'highlights' => array(
                'Đà Lạt 2 ngày',
                'Xe xuống Phan Thiết',
                'Gợi ý đồi cát sunrise',
                'Seafood Mũi Né',
            ),
            'itinerary' => array(
                array(
                    'day' => 1,
                    'title' => 'Đà Lạt',
                    'meals' => 'Trưa; Tối',
                    'transport' => array(
                        'car',
                    ),
                    'overnight' => 'Đà Lạt',
                    'content' => 'City + Datanla.',
                ),
                array(
                    'day' => 2,
                    'title' => 'Xuống Mũi Né',
                    'meals' => 'Sáng; Tối',
                    'transport' => array(
                        'car',
                        'bus',
                    ),
                    'overnight' => 'Mũi Né',
                    'content' => 'Xe ~3h xuống biển.',
                ),
                array(
                    'day' => 3,
                    'title' => 'Mũi Né — Tiễn',
                    'meals' => 'Sáng',
                    'transport' => array(
                        'car',
                    ),
                    'overnight' => null,
                    'content' => 'Tự do, tiễn.',
                ),
            ),
            'inclusions' => array(
                'Xe Đà Lạt',
                'HDV',
                'Xe nối Mũi Né',
            ),
            'exclusions' => array(
                'Lưu trú',
                'Jeep đồi cát',
            ),
            'notes' => array(),
            'faqs' => array(),
            'galleryCount' => 4,
            'priceFrom' => 2950000,
            'currency' => 'VND',
        ),
        array(
            'slug' => 'food-tour-cho-dem-nua-dem',
            'title' => 'Tour ẩm thực chợ đêm Đà Lạt — Buổi tối',
            'zoneSlug' => 'trung-tam-xuan-huong',
            'zone' => 'Trung tâm & Hồ Xuân Hương',
            'tourCode' => 'DL0.5D-01',
            'duration' => 'Nửa đêm',
            'days' => 1,
            'rating' => 4.8,
            'reviewCount' => 267,
            'badge' => 'Ăn uống',
            'featured' => true,
            'styles' => array(
                'day-trip',
                'am-thuc-cao-nguyen',
            ),
            'quote' => array(
                'text' => 'Bánh tráng nướng, sữa đậu nành nóng — ăn no mà vẫn muốn quay lại.',
                'author' => 'Food blogger Ken',
            ),
            'places' => array(
                'Chợ đêm Đà Lạt',
                'Hồ Xuân Hương',
            ),
            'start' => 'Đà Lạt',
            'end' => 'Đà Lạt',
            'highlightsIntro' => '18:00–22:00 — dẫn ăn 6–8 món đặc sản chợ đêm với HDV am thực.',
            'highlights' => array(
                'Bánh tráng nướng',
                'Sữa đậu nành',
                'Xiên nướng',
                'Kem bơ / dâu',
            ),
            'itinerary' => array(
                array(
                    'day' => 1,
                    'title' => 'Chợ đêm',
                    'meals' => 'Tối',
                    'transport' => array(
                        'walking',
                    ),
                    'overnight' => null,
                    'content' => 'Đi bộ chợ đêm, thử món, kể chuyện ẩm thực Đà Lạt.',
                ),
            ),
            'inclusions' => array(
                'HDV am thực',
                '7–8 món tasting',
            ),
            'exclusions' => array(
                'Đồ uống thêm',
                'Mua sắm',
            ),
            'notes' => array(
                'Báo dị ứng thực phẩm trước.',
            ),
            'faqs' => array(),
            'galleryCount' => 3,
            'priceFrom' => 380000,
            'currency' => 'VND',
        ),
        array(
            'slug' => 'tuyen-lam-truc-lam-robin-1-ngay',
            'title' => 'Tuyền Lâm — Trúc Lâm — Robin Hill 1 ngày',
            'zoneSlug' => 'tuyen-lam-truc-lam',
            'zone' => 'Tuyền Lâm & Trúc Lâm',
            'tourCode' => 'DL1D-06',
            'duration' => '1 ngày',
            'days' => 1,
            'rating' => 4.8,
            'reviewCount' => 178,
            'badge' => null,
            'featured' => true,
            'styles' => array(
                'day-trip',
                'gia-dinh-tre-em',
                'cap-doi-lang-man',
            ),
            'quote' => array(
                'text' => 'Cáp treo Robin Hill nhìn xuống hồ Tuyền Lâm — view đẹp nhất chuyến đi.',
                'author' => 'Chị Thùy Linh',
            ),
            'places' => array(
                'Robin Hill',
                'Trúc Lâm',
                'Hồ Tuyền Lâm',
            ),
            'start' => 'Đà Lạt',
            'end' => 'Đà Lạt',
            'highlightsIntro' => 'Combo cáp treo + thiền viện + thuyền hồ — yên tĩnh và đẹp.',
            'highlights' => array(
                'Cáp treo Robin Hill',
                'Thiền viện Trúc Lâm',
                'Thuyền Tuyền Lâm',
                'Paddle boat (tuỳ chọn)',
            ),
            'itinerary' => array(
                array(
                    'day' => 1,
                    'title' => 'Tuyền Lâm circuit',
                    'meals' => 'Trưa',
                    'transport' => array(
                        'car',
                        'cable_car',
                        'boat',
                    ),
                    'overnight' => null,
                    'content' => 'Cáp treo xuống Trúc Lâm, thuyền hồ, trưa nhà hàng view hồ.',
                ),
            ),
            'inclusions' => array(
                'Xe',
                'Vé cáp treo',
                'Thuyền',
                'HDV',
            ),
            'exclusions' => array(
                'Đồ uống',
            ),
            'notes' => array(),
            'faqs' => array(),
            'galleryCount' => 4,
            'priceFrom' => 680000,
            'currency' => 'VND',
        ),
        array(
            'slug' => 'chup-anh-trang-mat-da-lat',
            'title' => 'Tour chụp ảnh & trăng mật Đà Lạt — 1 ngày',
            'zoneSlug' => 'trung-tam-xuan-huong',
            'zone' => 'Trung tâm & Hồ Xuân Hương',
            'tourCode' => 'DL1D-07',
            'duration' => '1 ngày',
            'days' => 1,
            'rating' => 4.9,
            'reviewCount' => 92,
            'badge' => 'Pre-wedding',
            'featured' => false,
            'styles' => array(
                'day-trip',
                'cap-doi-lang-man',
                'festival-hoa',
            ),
            'quote' => array(
                'text' => 'Photographer biết góc hồ sương sớm — ảnh không cần filter.',
                'author' => 'Cặp đôi Huy — Mai',
            ),
            'places' => array(
                'Hồ Xuân Hương',
                'Đồi chè',
                'Vườn hoa',
            ),
            'start' => 'Đà Lạt',
            'end' => 'Đà Lạt',
            'highlightsIntro' => 'Xe + photographer half-day — góc hồ, đồi chè, vườn hoa cho couple.',
            'highlights' => array(
                'Photographer 4h',
                '3–4 địa điểm',
                'Gợi ý trang phục',
                'Gửi file chỉnh sơ',
            ),
            'itinerary' => array(
                array(
                    'day' => 1,
                    'title' => 'Tour chụp ảnh',
                    'meals' => null,
                    'transport' => array(
                        'car',
                    ),
                    'overnight' => null,
                    'content' => 'Sáng sớm hồ sương, trưa đồi chè/vườn hoa.',
                ),
            ),
            'inclusions' => array(
                'Xe',
                'Photographer',
                '50+ ảnh chỉnh màu',
            ),
            'exclusions' => array(
                'Makeup',
                'Trang phục',
            ),
            'notes' => array(
                'Đặt thêm wedding photo support trong Dịch vụ.',
            ),
            'faqs' => array(),
            'galleryCount' => 6,
            'priceFrom' => 2500000,
            'currency' => 'VND',
        ),
        array(
            'slug' => 'nau-an-da-lat-half-day',
            'title' => 'Lớp nấu ăn Đà Lạt — Nửa ngày',
            'zoneSlug' => 'trung-tam-xuan-huong',
            'zone' => 'Trung tâm & Hồ Xuân Hương',
            'tourCode' => 'DL0.5D-02',
            'duration' => 'Nửa ngày',
            'days' => 1,
            'rating' => 4.7,
            'reviewCount' => 48,
            'badge' => null,
            'featured' => false,
            'styles' => array(
                'day-trip',
                'am-thuc-cao-nguyen',
            ),
            'quote' => array(
                'text' => 'Tự làm bánh căn và salad atiso — mang recipe về nhà.',
                'author' => 'Khách Úc',
            ),
            'places' => array(
                'Farm atiso',
                'Bếp lớp học',
            ),
            'start' => 'Đà Lạt',
            'end' => 'Đà Lạt',
            'highlightsIntro' => 'Chợ → nấu 3 món đặc trưng (atiso, rau củ cao nguyên) — ăn trưa thành quả.',
            'highlights' => array(
                'Tour chợ sáng',
                'Nấu 3 món',
                'Recipe card',
                'Ăn trưa',
            ),
            'itinerary' => array(
                array(
                    'day' => 1,
                    'title' => 'Cooking class',
                    'meals' => 'Trưa',
                    'transport' => array(
                        'car',
                    ),
                    'overnight' => null,
                    'content' => '08:00 chợ, 09:30 nấu, 12:00 ăn.',
                ),
            ),
            'inclusions' => array(
                'Nguyên liệu',
                'Chef HDV',
                'Trưa',
            ),
            'exclusions' => array(),
            'notes' => array(
                'Vegetarian on request.',
            ),
            'faqs' => array(),
            'galleryCount' => 3,
            'priceFrom' => 550000,
            'currency' => 'VND',
        ),
    ),

    'cruises' => array(
        array(
            'slug' => 'thuyen-picnic-hoang-hon-tuyen-lam',
            'title' => 'Thuyền picnic hoàng hôn hồ Tuyền Lâm',
            'typeSlug' => 'thuyen-ho-tuyen-lam', 'typeName' => 'Thuyền hồ Tuyền Lâm',
            'tourCode' => 'LK1D-01', 'duration' => '3 giờ', 'days' => 1,
            'rating' => 4.9, 'reviewCount' => 112, 'badge' => 'Lãng mạn',
            'styles' => array('romantic', 'honeymoon', 'photography'),
            'quote' => array('text' => 'Hoàng hôn trên hồ núi — yên tĩnh hơn mọi nhà hàng view.', 'author' => 'Chị Diệu Linh'),
            'places' => array('Hồ Tuyền Lâm', 'Rừng thông bao quanh'),
            'start' => 'Bến thuyền Tuyền Lâm', 'end' => 'Bến thuyền Tuyền Lâm',
            'departurePort' => 'Bến Tuyền Lâm', 'boatClass' => 'Thuyền gỗ mái che', 'nightsOnBoard' => 0,
            'cabinTypes' => array(),
            'highlightsIntro' => '16:30–19:30 — thuyền riêng hoặc nhóm nhỏ, picnic nhẹ và ngắm hoàng hôn.',
            'highlights' => array('Thuyền mái che ấm', 'Picnic trà & bánh', 'Nhóm max 8 khách', 'Góc chụp hoàng hôn'),
            'itinerary' => array(array('day' => 1, 'title' => 'Sunset cruise', 'meals' => 'Picnic nhẹ', 'transport' => array('boat'), 'overnight' => null,
                'content' => 'Lên thuyền chiều, dạo hồ, dừng điểm view, về trước tối.')),
            'inclusions' => array('Thuyền', 'Picnic', 'HDV'), 'exclusions' => array('Đồ uống có cồn', 'Xe từ trung tâm'),
            'notes' => array('Phụ thuộc thời tiết — đổi ngày khi mưa lớn.'), 'faqs' => array(),
            'galleryCount' => 4, 'priceFrom' => 480000.0, 'currency' => 'VND',
        ),
        array(
            'slug' => 'sup-kayak-sang-tuyen-lam',
            'title' => 'SUP & kayak buổi sáng hồ Tuyền Lâm',
            'typeSlug' => 'sup-kayak-tuyen-lam', 'typeName' => 'SUP & kayak Tuyền Lâm',
            'tourCode' => 'LK1D-02', 'duration' => '2.5 giờ', 'days' => 1,
            'rating' => 4.8, 'reviewCount' => 67, 'badge' => null,
            'styles' => array('adventure', 'day-trip', 'balanced'),
            'quote' => array('text' => 'Nước hồ lặng sáng sớm — SUP dễ hơn tưởng tượng.', 'author' => 'Anh Felix'),
            'places' => array('Hồ Tuyền Lâm'),
            'start' => 'Bến Tuyền Lâm', 'end' => 'Bến Tuyền Lâm',
            'departurePort' => 'Bến Tuyền Lâm', 'boatClass' => 'SUP / kayak đôi', 'nightsOnBoard' => 0,
            'cabinTypes' => array(),
            'highlightsIntro' => '06:30–09:00 — sương mỏng trên mặt hồ, phù hợp người mới.',
            'highlights' => array('SUP hoặc kayak', 'Áo phao & briefing', 'HDV kèm', 'Nước & snack'),
            'itinerary' => array(array('day' => 1, 'title' => 'Chèo kayak buổi sáng', 'meals' => 'Đồ ăn nhẹ', 'transport' => array('kayak'), 'overnight' => null,
                'content' => 'Briefing, chèo 1.5h quanh bờ rừng thông, về bến.')),
            'inclusions' => array('Thiết bị', 'HDV', 'Snack'), 'exclusions' => array('Xe đón'),
            'notes' => array('Biết bơi cơ bản khuyến nghị.'), 'faqs' => array(),
            'galleryCount' => 3, 'priceFrom' => 350000.0, 'currency' => 'VND',
        ),
        array(
            'slug' => 'thuyen-cau-ca-tuyen-lam',
            'title' => 'Thuyền câu cá & dã ngoại Tuyền Lâm — Nửa ngày',
            'typeSlug' => 'thuyen-ho-tuyen-lam', 'typeName' => 'Thuyền hồ Tuyền Lâm',
            'tourCode' => 'LK0.5D-03', 'duration' => '4 giờ', 'days' => 1,
            'rating' => 4.7, 'reviewCount' => 41, 'badge' => null,
            'styles' => array('family', 'day-trip', 'balanced'),
            'quote' => array('text' => 'Con câu được cá — nướng luôn trên thuyền, vui hết cỡ.', 'author' => 'Gia đình anh Tùng'),
            'places' => array('Hồ Tuyền Lâm'),
            'start' => 'Bến Tuyền Lâm', 'end' => 'Bến Tuyền Lâm',
            'departurePort' => 'Bến Tuyền Lâm', 'boatClass' => 'Thuyền câu', 'nightsOnBoard' => 0,
            'cabinTypes' => array(),
            'highlightsIntro' => 'Trải nghiệm địa phương — câu cá hồ, nướng trên thuyền, không cần kinh nghiệm.',
            'highlights' => array('Cần câu & mồi', 'Nướng trên thuyền', 'HDV địa phương', 'Trà nóng'),
            'itinerary' => array(array('day' => 1, 'title' => 'Fishing boat', 'meals' => 'Brunch nhẹ', 'transport' => array('boat'), 'overnight' => null,
                'content' => 'Sáng ra giữa hồ câu, nướng cá, về trưa.')),
            'inclusions' => array('Thuyền', 'Dụng cụ câu', 'HDV'), 'exclusions' => array('Rượu'),
            'notes' => array('Không đảm bảo số lượng cá — trải nghiệm là chính.'), 'faqs' => array(),
            'galleryCount' => 3, 'priceFrom' => 420000.0, 'currency' => 'VND',
        ),
    ),

    'blog_categories' => array(
        array('slug' => 'trung-tam-xuan-huong', 'name' => 'Trung tâm Đà Lạt', 'zoneSlug' => 'trung-tam-xuan-huong', 'count' => 4),
        array('slug' => 'cau-dat', 'name' => 'Cầu Đất & cà phê', 'zoneSlug' => 'cau-dat', 'count' => 2),
        array('slug' => 'langbiang-ta-nung', 'name' => 'Langbiang & phiêu lưu', 'zoneSlug' => 'langbiang-ta-nung', 'count' => 2),
        array('slug' => 'di-chuyen', 'name' => 'Di chuyển & cửa ngõ', 'zoneSlug' => 'trung-tam-xuan-huong', 'count' => 2),
    ),

    'popular_keywords' => array(
        'Kinh nghiệm du lịch Đà Lạt', 'Đà Lạt mùa nào đẹp', 'Từ Sài Gòn đi Đà Lạt', 'Canyoning Datanla',
        'Tour cà phê Cầu Đất', 'Festival hoa Đà Lạt', 'Ăn gì ở Đà Lạt', 'Sân bay Liên Khương đưa đón',
        'Langbiang trekking', 'Homestay Đà Lạt khu nào', 'Limousine Sài Gòn Đà Lạt', 'Đà Lạt 3 ngày 2 đêm',
    ),

    'articles' => array(
        array(
            'slug' => 'tu-sai-gon-di-da-lat-the-nao',
            'title' => 'Từ Sài Gòn đi Đà Lạt thế nào? Xe giường, limousine hay bay',
            'zoneSlug' => 'trung-tam-xuan-huong', 'zone' => 'Trung tâm & Hồ Xuân Hương',
            'category' => 'Di chuyển & cửa ngõ', 'categorySlug' => 'di-chuyen',
            'tags' => array('Di chuyển thế nào?', 'Mẹo du lịch'),
            'author' => 'Minh Trí', 'publishedAt' => '05/06/2026', 'updatedAt' => '20/07/2026',
            'views' => 2140, 'rating' => 4.9, 'ratingCount' => 58,
            'excerpt' => 'So sánh xe giường nằm (5–7h), limousine (5–6h) và bay DLI (~1h + 45 phút ra thành phố) — phù hợp từng kiểu khách.',
            'content' => array(
                array('type' => 'p', 'text' => 'Phần lớn khách Đà Lạt cuối tuần đến từ TP.HCM. Ba phương án phổ biến: xe giường nằm đêm, limousine ban ngày, hoặc bay sân bay Liên Khương (DLI) cách trung tâm ~30km.'),
                array('type' => 'h2', 'id' => 'xe-giuong', 'text' => 'I. Xe giường nằm Sài Gòn — Đà Lạt'),
                array('type' => 'p', 'text' => 'Khởi hành tối (19h–22h), tới sáng hôm sau — tiết kiệm 1 đêm lưu trú, giá 250.000–400.000đ/ghế. Phù hợp sinh viên và nhóm bạn.'),
                array('type' => 'h2', 'id' => 'limousine', 'text' => 'II. Limousine 9–16 chỗ'),
                array('type' => 'p', 'text' => 'Đón quận trung tâm Sài Gòn, mất 5–6 giờ, ghế massage — phổ biến với gia đình và cặp đôi. Nên đặt trước thứ 6.'),
                array('type' => 'h2', 'id' => 'bay-dli', 'text' => 'III. Bay DLI + shuttle'),
                array('type' => 'p', 'text' => 'Bay ~50 phút, giá vé dao động theo mùa. Cộng shuttle sân bay ~30km (~45 phút) — tổng thời gian có thể ngang limousine nếu tính check-in.'),
            ),
            'faqs' => array(array('q' => 'Nên đi đêm hay đi sáng?', 'a' => 'Đi đêm tiết kiệm thời gian; đi sáng ngắm cảnh đèo và tránh say xe nếu bạn nhạy cảm.')),
            'galleryCount' => 4,
        ),
        array(
            'slug' => 'da-lat-mua-nao-dep-nhat',
            'title' => 'Đà Lạt mùa nào đẹp nhất? Cẩm nang khí hậu cao nguyên',
            'zoneSlug' => 'trung-tam-xuan-huong', 'zone' => 'Trung tâm & Hồ Xuân Hương',
            'category' => 'Trung tâm Đà Lạt', 'categorySlug' => 'trung-tam-xuan-huong',
            'tags' => array('Mẹo du lịch', 'Chơi gì, xem gì?'),
            'author' => 'Lan Hương', 'publishedAt' => '12/06/2026', 'updatedAt' => '28/07/2026',
            'views' => 1890, 'rating' => 4.9, 'ratingCount' => 44,
            'excerpt' => 'Mát quanh năm nhưng mỗi mùa một vẻ: hoa xuân, mưa phùn hè, sương mù thu, festival hoa cuối năm.',
            'content' => array(
                array('type' => 'p', 'text' => 'Đà Lạt ~1.500m so với mực nước biển — nhiệt độ trung bình 18–24°C. Mùa khô (tháng 11–4) dễ đi; mùa mưa (5–10) sương mù dày, cảnh đẹp nhưng canyoning có thể hủy.'),
                array('type' => 'h2', 'id' => 'festival-hoa', 'text' => 'Festival hoa (cuối tháng 12)'),
                array('type' => 'p', 'text' => 'Đường hoa, parade và giá phòng tăng — đặt xe và tư vấn lưu trú trước 3–4 tuần.'),
            ),
            'faqs' => array(), 'galleryCount' => 5,
        ),
        array(
            'slug' => 'an-gi-o-da-lat',
            'title' => 'Ăn gì ở Đà Lạt? Đặc sản chợ đêm & quán địa phương',
            'zoneSlug' => 'trung-tam-xuan-huong', 'zone' => 'Trung tâm & Hồ Xuân Hương',
            'category' => 'Trung tâm Đà Lạt', 'categorySlug' => 'trung-tam-xuan-huong',
            'tags' => array('Ăn gì, uống gì?'),
            'author' => 'Minh Trí', 'publishedAt' => '18/06/2026', 'updatedAt' => '02/08/2026',
            'views' => 1560, 'rating' => 4.8, 'ratingCount' => 36,
            'excerpt' => 'Bánh tráng nướng, lẩu gà lá é, atiso, dâu tây và cà phê — bản đồ ẩm thực cao nguyên.',
            'content' => array(
                array('type' => 'p', 'text' => 'Ẩm thực Đà Lạt gắn với rau củ mát, atiso và sữa. Chợ đêm là điểm bắt buộc cho tour ẩm thực.'),
                array('type' => 'ul', 'items' => array('Bánh tráng nướng', 'Lẩu gà lá é', 'Bánh căn', 'Atiso & salad', 'Dâu tây & sữa chua')),
            ),
            'faqs' => array(), 'galleryCount' => 4,
        ),
        array(
            'slug' => 'festival-hoa-da-lat-huong-dan',
            'title' => 'Festival hoa Đà Lạt: lịch, vé và mẹo tránh đông',
            'zoneSlug' => 'trung-tam-xuan-huong', 'zone' => 'Trung tâm & Hồ Xuân Hương',
            'category' => 'Trung tâm Đà Lạt', 'categorySlug' => 'trung-tam-xuan-huong',
            'tags' => array('Chơi gì, xem gì?', 'Mẹo du lịch'),
            'author' => 'Lan Hương', 'publishedAt' => '01/11/2026', 'updatedAt' => '15/11/2026',
            'views' => 980, 'rating' => 4.9, 'ratingCount' => 28,
            'excerpt' => 'Sự kiện cuối năm (thường 20–31/12) — đường hoa, show và lưu lượng khách tăng mạnh.',
            'content' => array(
                array('type' => 'p', 'text' => 'Festival hoa là lý do nhiều khách chọn Đà Lạt dịp Giáng sinh — nên đặt limousine và phòng sớm.'),
            ),
            'faqs' => array(), 'galleryCount' => 4,
        ),
        array(
            'slug' => 'cau-dat-cafe-huong-dan',
            'title' => 'Tour cà phê Cầu Đất: Arabica, cupping và mùa thu hoạch',
            'zoneSlug' => 'cau-dat', 'zone' => 'Cao nguyên Cầu Đất',
            'category' => 'Cầu Đất & cà phê', 'categorySlug' => 'cau-dat',
            'tags' => array('Chơi gì, xem gì?', 'Chọn tour nào?'),
            'author' => 'Phạm Thị Liên', 'publishedAt' => '22/06/2026', 'updatedAt' => '10/07/2026',
            'views' => 1120, 'rating' => 4.9, 'ratingCount' => 31,
            'excerpt' => 'Cầu Đất ~1.650m — vùng cà phê Arabica nổi tiếng nhất Lâm Đồng.',
            'content' => array(
                array('type' => 'p', 'text' => 'Tour farm 3–4 giờ: vườn → sơ chế → cupping. Mùa thu hoạch chính tháng 11–1.'),
            ),
            'faqs' => array(), 'galleryCount' => 5,
        ),
        array(
            'slug' => 'canyoning-datanla-kinh-nghiem',
            'title' => 'Canyoning Datanla: chuẩn bị, độ tuổi và an toàn',
            'zoneSlug' => 'datanla-robin-hill', 'zone' => 'Datanla & Robin Hill',
            'category' => 'Langbiang & phiêu lưu', 'categorySlug' => 'langbiang-ta-nung',
            'tags' => array('Chơi gì, xem gì?', 'Mẹo du lịch'),
            'author' => 'Lan Hương', 'publishedAt' => '08/07/2026', 'updatedAt' => '25/07/2026',
            'views' => 876, 'rating' => 4.9, 'ratingCount' => 27,
            'excerpt' => 'Hoạt động mạo hiểm phổ biến nhất Đà Lạt — cần biết bơi và HLV chứng nhận.',
            'content' => array(
                array('type' => 'p', 'text' => 'Canyoning gồm leo thác, zipline và nhảy nước — 3–4 giờ với HLV. Hủy khi mưa lớn vì an toàn.'),
            ),
            'faqs' => array(array('q' => 'Trẻ em đi được không?', 'a' => 'Thường từ 16 tuổi; một số nhà vận hành nhận 14 tuổi nếu có phụ huynh và bơi tốt.')),
            'galleryCount' => 4,
        ),
        array(
            'slug' => 'o-dau-o-da-lat-tu-van',
            'title' => 'Ở đâu ở Đà Lạt? Gợi ý khu homestay, villa và resort',
            'zoneSlug' => 'trung-tam-xuan-huong', 'zone' => 'Trung tâm & Hồ Xuân Hương',
            'category' => 'Trung tâm Đà Lạt', 'categorySlug' => 'trung-tam-xuan-huong',
            'tags' => array('Ở đâu?', 'Mẹo du lịch'),
            'author' => 'Lê Thu Hương', 'publishedAt' => '15/07/2026', 'updatedAt' => '01/08/2026',
            'views' => 1340, 'rating' => 4.8, 'ratingCount' => 41,
            'excerpt' => 'Trung tâm tiện ăn uống, Tà Nung yên tĩnh, Cầu Đất mát — chọn khu ở phù hợp hành trình.',
            'content' => array(
                array('type' => 'p', 'text' => 'Trung tâm tiện ăn uống; Tà Nung yên, view đồi; Cầu Đất mát và farm stay; khu ven hồ Tuyền Lâm lãng mạn. Liên hệ :brand để nhận gợi ý theo ngân sách.'),
            ),
            'faqs' => array(), 'galleryCount' => 3,
        ),
        array(
            'slug' => 'langbiang-trekking-meo',
            'title' => 'Langbiang trekking & jeep: chọn cung nào?',
            'zoneSlug' => 'langbiang-ta-nung', 'zone' => 'Langbiang & Tà Nung',
            'category' => 'Langbiang & phiêu lưu', 'categorySlug' => 'langbiang-ta-nung',
            'tags' => array('Chơi gì, xem gì?', 'Chọn tour nào?'),
            'author' => 'Phạm Thị Liên', 'publishedAt' => '20/07/2026', 'updatedAt' => '05/08/2026',
            'views' => 720, 'rating' => 4.8, 'ratingCount' => 22,
            'excerpt' => 'Jeep nhanh, trek chân thực hơn — đỉnh 2.167m view toàn thành phố trong sương.',
            'content' => array(
                array('type' => 'p', 'text' => 'Jeep phù hợp người ít thời gian; trekking 2–3h cho người thích thử thách. Mang áo ấm — đỉnh lạnh hơn thành phố 5–8°C.'),
            ),
            'faqs' => array(), 'galleryCount' => 4,
        ),
        array(
            'slug' => 'cuoi-tuan-sai-gon-da-lat-lich-trinh',
            'title' => 'Cuối tuần Sài Gòn — Đà Lạt: lịch trình 2 ngày gợi ý',
            'zoneSlug' => 'trung-tam-xuan-huong', 'zone' => 'Trung tâm & Hồ Xuân Hương',
            'category' => 'Di chuyển & cửa ngõ', 'categorySlug' => 'di-chuyen',
            'tags' => array('Chọn tour nào?', 'Mẹo du lịch'),
            'author' => 'Minh Trí', 'publishedAt' => '28/07/2026', 'updatedAt' => '12/08/2026',
            'views' => 1680, 'rating' => 4.9, 'ratingCount' => 52,
            'excerpt' => 'Tối thứ 6 ra Đà Lạt, chủ nhật về — gợi ý city tour, Cầu Đất và chợ đêm.',
            'content' => array(
                array('type' => 'p', 'text' => 'Combo phổ biến: limousine tối thứ 6, 2 đêm lưu trú, city tour + Cầu Đất + tour ẩm thực, limousine chiều chủ nhật về.'),
            ),
            'faqs' => array(), 'galleryCount' => 4,
        ),
        array(
            'slug' => 'da-lat-trang-mat-goi-y',
            'title' => 'Đà Lạt trăng mật: 5 trải nghiệm lãng mạn',
            'zoneSlug' => 'tuyen-lam-truc-lam', 'zone' => 'Tuyền Lâm & Trúc Lâm',
            'category' => 'Trung tâm Đà Lạt', 'categorySlug' => 'trung-tam-xuan-huong',
            'tags' => array('Chơi gì, xem gì?', 'Chuyến đi thế nào?'),
            'author' => 'Lê Thu Hương', 'publishedAt' => '02/08/2026', 'updatedAt' => '18/08/2026',
            'views' => 890, 'rating' => 4.9, 'ratingCount' => 24,
            'excerpt' => 'Thuyền hoàng hôn Tuyền Lâm, Lumiere, hoa surprise và photo tour — không cần resort 5 sao.',
            'content' => array(
                array('type' => 'ul', 'items' => array('Sunset boat Tuyền Lâm', 'Lumiere Light Garden', 'Vườn hoa & hoa surprise', 'Photo tour couple', 'Bữa tối view đồi thông')),
            ),
            'faqs' => array(), 'galleryCount' => 5,
        ),
    ),

    'testimonials' => array(
        array('name' => 'Nguyễn Minh Anh', 'country' => 'Việt Nam', 'flag' => '🇻🇳', 'rating' => 5.0,
            'quote' => 'Tour 3 ngày vừa đủ — cà phê Cầu Đất và chợ đêm là hai điểm nhớ nhất.', 'photos' => 5, 'trip' => 'Đà Lạt 3 ngày 2 đêm', 'avatar' => null, 'photoUrls' => array()),
        array('name' => 'Sarah Kim', 'country' => 'Hàn Quốc', 'flag' => '🇰🇷', 'rating' => 5.0,
            'quote' => 'Canyoning was thrilling but felt very safe — best activity in Vietnam for us.', 'photos' => 8, 'trip' => 'Canyoning Datanla', 'avatar' => null, 'photoUrls' => array()),
        array('name' => 'Trần Quốc Bảo', 'country' => 'Việt Nam', 'flag' => '🇻🇳', 'rating' => 4.9,
            'quote' => 'Tư vấn homestay Tà Nung đúng gu — view rừng thông, sáng dậy trong sương.', 'photos' => 4, 'trip' => 'Đà Lạt 4 ngày khám phá', 'avatar' => null, 'photoUrls' => array()),
        array('name' => 'Felix Müller', 'country' => 'Đức', 'flag' => '🇩🇪', 'rating' => 4.8,
            'quote' => 'Coffee tour at Cau Dat finally explained why Dalat beans taste different.', 'photos' => 6, 'trip' => 'Cầu Đất cafe tour', 'avatar' => null, 'photoUrls' => array()),
        array('name' => 'Diệu Linh', 'country' => 'Việt Nam', 'flag' => '🇻🇳', 'rating' => 5.0,
            'quote' => 'Thuyền hoàng hôn Tuyền Lâm — lãng mạn hơn mọi nhà hàng view.', 'photos' => 3, 'trip' => 'Sunset boat Tuyền Lâm', 'avatar' => null, 'photoUrls' => array()),
        array('name' => 'David Park', 'country' => 'Úc', 'flag' => '🇦🇺', 'rating' => 4.9,
            'quote' => 'Limousine from Saigon was smooth — they even helped us pick a villa area.', 'photos' => 7, 'trip' => 'Cuối tuần escape SGN', 'avatar' => null, 'photoUrls' => array()),
    ),

    'team' => array(
        array(
            'slug' => 'tran-van-lam', 'name' => 'Trần Văn Lâm', 'role' => 'Giám đốc điều hành',
            'bio' => 'Sinh ra tại Đà Lạt, hơn 14 năm thiết kế tour cao nguyên và kết nối homestay địa phương...',
            'phone' => '+84 263 388 8888', 'email' => 'lam.tran@hidalat.dev', 'area' => 'Đà Lạt, Lâm Đồng',
            'years_experience' => 14, 'languages' => array('Tiếng Việt', 'English'),
            'stat_clients' => 3200, 'stat_tours' => 680, 'stat_awards' => 4, 'is_verified' => true,
            'bio_html' => '<p>Sinh ra tại Đà Lạt, Trần Văn Lâm có hơn 14 năm kinh nghiệm thiết kế tour cao nguyên và tư vấn lưu trú theo khu vực.</p>',
            'bio_html_en' => '<p>Born in Da Lat, Van Lam has 14 years designing highland tours and advising stays by neighbourhood.</p>',
            'name_en' => 'Tran Van Lam', 'role_en' => 'Chief Executive Officer',
            'short_bio_en' => 'Born in Da Lat — 14 years designing highland tours and stay advice.',
            'achievements' => array('Xây dựng mạng 30+ homestay/villa đối tác trực tiếp', 'Đối tác tour cà phê Cầu Đất'),
            'skills' => array(array('skill' => 'Thiết kế tour cao nguyên', 'percent' => 96), array('skill' => 'Tư vấn lưu trú theo khu', 'percent' => 94)),
            'experiences' => array(array('title' => 'Giám đốc điều hành', 'company' => 'Hi Đà Lạt', 'items' => array('Chiến lược sản phẩm tour & dịch vụ Đà Lạt'))),
            'degrees' => array(array('title' => 'Cử nhân Quản trị Du lịch', 'school' => 'Đại học Đà Lạt', 'items' => array())),
        ),
        array(
            'slug' => 'nguyen-thi-hoa', 'name' => 'Nguyễn Thị Hoa', 'role' => 'Trưởng phòng thiết kế tour',
            'bio' => 'Chuyên gia lễ hội hoa, tour cà phê và lịch trình cặp đôi/trăng mật...',
            'phone' => '+84 263 388 8899', 'email' => 'hoa.nguyen@hidalat.dev', 'area' => 'Đà Lạt & Cầu Đất',
            'years_experience' => 10, 'languages' => array('Tiếng Việt', 'English', '한국어'),
            'stat_clients' => 2100, 'stat_tours' => 410, 'stat_awards' => 2, 'is_verified' => true,
            'bio_html' => '<p>Nguyễn Thị Hoa phụ trách sản phẩm hoa, cà phê và tour lãng mạn — biết từng mùa hoa nở trên cao nguyên.</p>',
            'bio_html_en' => '<p>Thi Hoa leads flower, coffee and romantic itineraries — she knows every bloom season on the plateau.</p>',
            'name_en' => 'Nguyen Thi Hoa', 'role_en' => 'Head of Tour Design',
            'short_bio_en' => 'Flower festival, coffee tours and honeymoon itineraries specialist.',
            'achievements' => array('Thiết kế tour Festival hoa được đánh giá 4.9/5'),
            'skills' => array(array('skill' => 'Tour cà phê & agri', 'percent' => 95), array('skill' => 'Itinerary lãng mạn', 'percent' => 93)),
            'experiences' => array(array('title' => 'Trưởng phòng thiết kế tour', 'company' => 'Hi Đà Lạt', 'items' => array('Sản phẩm hoa, cafe, couple'))),
            'degrees' => array(array('title' => 'Cử nhân Địa lý Du lịch', 'school' => 'Đại học Khoa học Tự nhiên', 'items' => array())),
        ),
        array(
            'slug' => 'le-quang-huy', 'name' => 'Lê Quang Huy', 'role' => 'Trưởng đội phiêu lưu & an toàn',
            'bio' => 'HLV canyoning chứng nhận, phụ trách Langbiang, Datanla và đối tác ATV/jeep...',
            'phone' => '+84 263 388 8877', 'email' => 'huy.le@hidalat.dev', 'area' => 'Langbiang & Datanla',
            'years_experience' => 11, 'languages' => array('Tiếng Việt', 'English'),
            'stat_clients' => 2800, 'stat_tours' => 520, 'stat_awards' => 3, 'is_verified' => true,
            'bio_html' => '<p>Lê Quang Huy là HLV canyoning và điều phối an toàn cho mọi hoạt động mạo hiểm tại Đà Lạt.</p>',
            'bio_html_en' => '<p>Quang Huy is a certified canyoning coach and safety lead for adventure activities in Da Lat.</p>',
            'name_en' => 'Le Quang Huy', 'role_en' => 'Head of Adventure & Safety',
            'short_bio_en' => 'Certified canyoning coach — safety lead for Datanla and Langbiang.',
            'achievements' => array('Hơn 400 chuyến canyoning không sự cố nghiêm trọng'),
            'skills' => array(array('skill' => 'Canyoning & rescue', 'percent' => 97), array('skill' => 'Điều phối jeep/ATV', 'percent' => 92)),
            'experiences' => array(array('title' => 'Trưởng đội phiêu lưu', 'company' => 'Hi Đà Lạt', 'items' => array('An toàn canyoning, trekking, jeep'))),
            'degrees' => array(array('title' => 'Chứng chỉ HLV canyoning', 'school' => 'Đối tác quốc tế', 'items' => array())),
        ),
        array(
            'slug' => 'pham-minh-chau', 'name' => 'Phạm Minh Châu', 'role' => 'Chuyên gia tư vấn cao cấp',
            'bio' => 'Tư vấn khách Sài Gòn cuối tuần, combo Nha Trang/Mũi Né và lưu trú homestay/villa...',
            'phone' => '+84 263 388 8866', 'email' => 'chau.pham@hidalat.dev', 'area' => 'Đà Lạt & TP.HCM',
            'years_experience' => 8, 'languages' => array('Tiếng Việt', 'English'),
            'stat_clients' => 1900, 'stat_tours' => 350, 'stat_awards' => 1, 'is_verified' => true,
            'bio_html' => '<p>Phạm Minh Châu là đầu mối tư vấn cho khách TP.HCM — limousine, bay DLI và gợi ý homestay theo ngân sách.</p>',
            'bio_html_en' => '<p>Minh Chau advises HCMC weekend guests — limousines, DLI flights and stay suggestions by budget.</p>',
            'name_en' => 'Pham Minh Chau', 'role_en' => 'Senior Travel Consultant',
            'short_bio_en' => 'HCMC weekend escapes, combos and homestay/villa advice.',
            'achievements' => array('Tư vấn 500+ kỳ nghỉ cuối tuần SGN—Đà Lạt (2022–2026)'),
            'skills' => array(array('skill' => 'Tư vấn khách miền Nam', 'percent' => 96), array('skill' => 'Combo đa điểm', 'percent' => 91)),
            'experiences' => array(array('title' => 'Chuyên gia tư vấn', 'company' => 'Hi Đà Lạt', 'items' => array('Khách Sài Gòn & combo biển'))),
            'degrees' => array(array('title' => 'Cử nhân Marketing Du lịch', 'school' => 'UEH', 'items' => array())),
        ),
    ),

    'videos' => array(
        array('title' => 'Sương sớm hồ Xuân Hương', 'description' => 'Đà Lạt buổi sáng — nhiệt độ 16°C.', 'date' => '10/07/2026', 'duration' => '06:40', 'tag' => 'Trung tâm',
            'image' => 'https://i.ytimg.com/vi/DL00000001/hqdefault.jpg', 'imageSrcset' => null,
            'embedUrl' => 'https://www.youtube.com/embed/DL00000001?autoplay=1&rel=0', 'provider' => 'youtube', 'youtubeId' => 'DL00000001'),
        array('title' => 'Canyoning Datanla — góc nhìn GoPro', 'description' => 'Nhảy thác an toàn với HLV.', 'date' => '22/06/2026', 'duration' => '08:15', 'tag' => 'Phiêu lưu',
            'image' => 'https://i.ytimg.com/vi/DL00000002/hqdefault.jpg', 'imageSrcset' => null,
            'embedUrl' => 'https://www.youtube.com/embed/DL00000002?autoplay=1&rel=0', 'provider' => 'youtube', 'youtubeId' => 'DL00000002'),
    ),

    'gallery_albums' => array(
        array('title' => 'Festival hoa 2025 — đường hoa', 'photos' => 24, 'date' => '12/2025'),
        array('title' => 'Cầu Đất cupping tour', 'photos' => 18, 'date' => '08/2026'),
        array('title' => 'Cặp đôi Tuyền Lâm sunset', 'photos' => 15, 'date' => '07/2026'),
    ),

    'usps' => array(
        array('icon' => 'compass', 'sort' => 0,
            'vi' => array('title' => 'am hiểu cao nguyên như người Đà Lạt', 'description' => 'Đội ngũ bản địa biết mùa hoa, khu homestay phù hợp và lịch canyoning an toàn.'),




'en' => array('title' => 'true highland locals', 'description' => 'Our team knows flower seasons, the right stay areas and safe canyoning windows.')),
        array('icon' => 'refund', 'sort' => 1,
            'vi' => array('title' => 'báo giá minh bạch, không phí ẩn', 'description' => 'Giá tour và dịch vụ liệt kê rõ từng hạng mục; lưu trú chọn theo khu vực và ngân sách của bạn.'),




'en' => array('title' => 'transparent pricing, no hidden fees', 'description' => 'Clear line-item quotes for tours and services; stays matched to your area and budget.')),
        array('icon' => 'flower', 'sort' => 2,
            'vi' => array('title' => 'từ hoa đến cà phê & phiêu lưu', 'description' => 'Một đầu mối cho festival hoa, farm Cầu Đất, canyoning và combo biển.'),




'en' => array('title' => 'flowers to coffee & adventure', 'description' => 'One contact for flower festival, Cau Dat farms, canyoning and beach combos.')),
        array('icon' => 'support', 'sort' => 3,
            'vi' => array('title' => 'hỗ trợ 24/7 trên cao nguyên', 'description' => 'Hotline khi sương mù, trượt lịch canyoning hoặc cần đổi limousine về Sài Gòn.'),




'en' => array('title' => '24/7 highland support', 'description' => 'Hotline for fog delays, canyoning reschedules or changing your return limousine.')),
    ),

    'offices' => array(
        array('city' => 'Đà Lạt, Lâm Đồng', 'address' => 'Số 42 đường 3/2, phường Xuân Hương', 'phone' => '+84 263 388 8888'),
        array('city' => 'TP.HCM (đặt tour)', 'address' => 'Quận 1 — văn phòng đại diện', 'phone' => '+84 28 3999 6666'),
    ),

    'values' => array(
        array('vi' => array('name' => 'Tận tâm', 'desc' => 'Mỗi chuyến đi được chăm như khách mời tại nhà'),



'en' => array('name' => 'Dedication', 'desc' => 'Every trip is hosted like a guest at home')),
        array('vi' => array('name' => 'Am hiểu Đà Lạt', 'desc' => 'Sinh ra trên cao nguyên — hiểu từng mùa sương'),



'en' => array('name' => 'Da Lat expertise', 'desc' => 'Raised on the plateau — we know every mist season')),
        array('vi' => array('name' => 'Chân thành', 'desc' => 'Tư vấn đúng nhu cầu — không ép mua thêm dịch vụ'),



'en' => array('name' => 'Sincerity', 'desc' => 'Advice that fits your trip — never upselling you')),
        array('vi' => array('name' => 'An toàn', 'desc' => 'Phiêu lưu có HLV chứng nhận và kế hoạch dự phòng'),



'en' => array('name' => 'Safety', 'desc' => 'Adventure with certified guides and backup plans')),
    ),
    'value_definitions' => array(
        array('vi' => array('name' => 'Tận tâm', 'desc' => 'Mỗi chuyến đi được chăm như khách mời tại nhà'),



'en' => array('name' => 'Dedication', 'desc' => 'Every trip is hosted like a guest at home')),
        array('vi' => array('name' => 'Am hiểu Đà Lạt', 'desc' => 'Sinh ra trên cao nguyên — hiểu từng mùa sương'),



'en' => array('name' => 'Da Lat expertise', 'desc' => 'Raised on the plateau — we know every mist season')),
        array('vi' => array('name' => 'Chân thành', 'desc' => 'Tư vấn đúng nhu cầu — không ép mua thêm dịch vụ'),



'en' => array('name' => 'Sincerity', 'desc' => 'Advice that fits your trip — never upselling you')),
        array('vi' => array('name' => 'An toàn', 'desc' => 'Phiêu lưu có HLV chứng nhận và kế hoạch dự phòng'),



'en' => array('name' => 'Safety', 'desc' => 'Adventure with certified guides and backup plans')),
    ),

    'reasons' => array(
        array('vi' => array('title' => 'HDV & HLV bản địa', 'desc' => 'Người Đà Lạt dẫn tour, canyoning và tư vấn khu ở.'),



'en' => array('title' => 'Local guides & coaches', 'desc' => 'Da Lat natives lead tours, canyoning and stay-area advice.')),
        array('vi' => array('title' => 'Lưu trú đúng khu, đúng ngân sách', 'desc' => 'Homestay, villa, resort — trung tâm, Tà Nung, Cầu Đất, ven hồ Tuyền Lâm.'),



'en' => array('title' => 'Stays matched to your trip', 'desc' => 'Homestays, villas and resorts — centre, Ta Nung, Cau Dat, Tuyen Lam lakeside.')),
        array('vi' => array('title' => 'Một đầu mối xe + bay + tour', 'desc' => 'Limousine Sài Gòn, shuttle Đà Lạt và tour gộp một báo giá.'),



'en' => array('title' => 'One contact: bus, flights, tours', 'desc' => 'limo Sài Gònusines, DLI shuttles and tours in one quote.')),
        array('vi' => array('title' => 'Hỗ trợ 24/7', 'desc' => 'Hotline khi thời tiết đổi lịch canyoning hoặc sương mù.'),



'en' => array('title' => '24/7 support', 'desc' => 'Hotline when weather shifts canyoning or fog delays travel.')),
    ),
    'reason_definitions' => array(
        array('vi' => array('title' => 'HDV & HLV bản địa', 'desc' => 'Người Đà Lạt dẫn tour, canyoning và tư vấn khu ở.'),



'en' => array('title' => 'Local guides & coaches', 'desc' => 'Da Lat natives lead tours, canyoning and stay-area advice.')),
        array('vi' => array('title' => 'Lưu trú đúng khu, đúng ngân sách', 'desc' => 'Homestay, villa, resort — trung tâm, Tà Nung, Cầu Đất, ven hồ Tuyền Lâm.'),



'en' => array('title' => 'Stays matched to your trip', 'desc' => 'Homestays, villas and resorts — centre, Ta Nung, Cau Dat, Tuyen Lam lakeside.')),
        array('vi' => array('title' => 'Một đầu mối xe + bay + tour', 'desc' => 'Limousine Sài Gòn, shuttle Đà Lạt và tour gộp một báo giá.'),



'en' => array('title' => 'One contact: bus, flights, tours', 'desc' => 'limo Sài Gònusines, DLI shuttles and tours in one quote.')),
        array('vi' => array('title' => 'Hỗ trợ 24/7', 'desc' => 'Hotline khi thời tiết đổi lịch canyoning hoặc sương mù.'),



'en' => array('title' => '24/7 support', 'desc' => 'Hotline when weather shifts canyoning or fog delays travel.')),
    ),

    'reference_persons' => array(
        array('name' => 'Ms. Sarah Kim', 'country' => 'Hàn Quốc', 'email' => 'sarah@hidalat.example', 'phone' => '+82 10 9876 5432', 'skype' => 'sarah.kim.dalat', 'image' => null, 'imageSrcset' => null),
        array('name' => 'Mr. Felix Müller', 'country' => 'Đức', 'email' => 'felix@hidalat.example', 'phone' => '+49 170 1234567', 'skype' => 'felix.muller.travel', 'image' => null, 'imageSrcset' => null),
    ),

    'about_page' => array(
        'vi' => array(
            'seo_title' => 'Về chúng tôi — Hi Đà Lạt, kết nối du khách với cao nguyên Lâm Đồng',
            'seo_description' => 'Câu chuyện, sứ mệnh và đội ngũ Hi Đà Lạt — tour, dịch vụ và tư vấn lưu trú theo khu vực.',
            'page_title' => 'Về chúng tôi',
            'page_subtitle' => 'Thành phố ngàn hoa — thiết kế bởi người bản địa yêu cao nguyên',
            'banner' => array('src' => null, 'srcset' => null, 'alt' => 'Đội ngũ Hi Đà Lạt'),
            'mission' => array('title' => 'Sứ mệnh', 'text' => 'Mang đến hành trình chân thật trên cao nguyên — từ hoa, cà phê đến phiêu lưu — và giúp mỗi khách chọn tour, dịch vụ lẫn chỗ ở phù hợp trong một hành trình thống nhất.', 'image' => null, 'imageSrcset' => null),
            'vision' => array('title' => 'Tầm nhìn', 'text' => 'Trở thành cầu nối tin cậy nhất giữa du khách và Đà Lạt — nơi mỗi người cảm nhận được "mùa xuân vĩnh cửu" của thành phố.', 'image' => null, 'imageSrcset' => null),
            'sales_policy' => array('title' => 'Chính sách minh bạch', 'content' => 'Báo giá tour và dịch vụ liệt kê rõ từng hạng mục. Hỗ trợ đặt lưu trú theo khu và ngân sách. Đổi ngày linh hoạt khi canyoning/trekking hủy vì thời tiết.', 'cta_label' => 'Hỏi thêm', 'cta_url' => null, 'image' => null, 'imageSrcset' => null),
            'values_section' => array('title' => 'Giá trị cốt lõi', 'hub_label' => 'Giá trị', 'eyebrow' => 'Điều chúng tôi tin', 'subtitle' => 'Bốn giá trị dẫn dắt mọi hành trình trên cao nguyên.'),
            'reasons_section' => array('title' => 'Vì sao chọn Hi Đà Lạt?', 'eyebrow' => 'Lý do', 'subtitle' => 'Bản địa, minh bạch, an toàn phiêu lưu.', 'cta_label' => 'Bắt đầu hành trình', 'cta_url' => null, 'image' => null, 'imageSrcset' => null),
            'reference_section' => array('title' => 'Đại diện nước ngoài', 'eyebrow' => 'Mạng lưới', 'subtitle' => 'Trao đổi bằng ngôn ngữ của bạn với đại diện Hi Đà Lạt.'),
        ),




'en' => array(
            'seo_title' => 'About us — Hi Da Lat, connecting travellers with Lam Dong highlands',
            'seo_description' => 'Our story, mission and team — tours, services and stay advice by area.',
            'page_title' => 'About us',
            'page_subtitle' => 'City of a thousand flowers — designed by locals who love the plateau',
            'banner' => array('src' => null, 'srcset' => null, 'alt' => 'Hi Da Lat team'),
            'mission' => array('title' => 'Mission', 'text' => 'Deliver authentic highland journeys — from flowers and coffee to adventure — and help every guest book tours, services and stays in one coherent trip.', 'image' => null, 'imageSrcset' => null),
            'vision' => array('title' => 'Vision', 'text' => 'The most trusted bridge between travellers and Da Lat — where everyone feels the city\'s eternal spring.', 'image' => null, 'imageSrcset' => null),
            'sales_policy' => array('title' => 'Transparent policy', 'content' => 'Clear line-item quotes for tours and services. Stay booking support by area and budget. Flexible rescheduling when weather cancels adventure activities.', 'cta_label' => 'Ask us', 'cta_url' => null, 'image' => null, 'imageSrcset' => null),
            'values_section' => array('title' => 'Core values', 'hub_label' => 'Values', 'eyebrow' => 'What we believe', 'subtitle' => 'Four values guiding every highland itinerary.'),
            'reasons_section' => array('title' => 'Why Hi Da Lat?', 'eyebrow' => 'Why us', 'subtitle' => 'Local expertise, clear pricing, safe adventure.', 'cta_label' => 'Start your journey', 'cta_url' => null, 'image' => null, 'imageSrcset' => null),
            'reference_section' => array('title' => 'Representatives abroad', 'eyebrow' => 'Network', 'subtitle' => 'Speak in your language with Hi Da Lat representatives.'),
        ),
    ),

    'hero_pills' => array(
        array('zone_slug' => 'cau-dat', 'vi' => array('label' => 'Cầu Đất cà phê'),



'en' => array('label' => 'Cau Dat coffee'), 'url' => '/diem-den/cau-dat'),
        array('zone_slug' => 'langbiang-ta-nung', 'vi' => array('label' => 'Langbiang'),



'en' => array('label' => 'Langbiang'), 'url' => '/diem-den/langbiang-ta-nung'),
    ),

    'home_sections' => array(
        'company_intro' => array(
            'vi' => array('key' => 'company_intro', 'eyebrow' => 'Chuyên gia Đà Lạt', 'title' => 'Mùa xuân vĩnh cửu trên cao nguyên', 'subtitle' => null,
                'body' => 'Hi Đà Lạt là nền tảng du lịch Đà Lạt do người bản địa xây dựng — <strong class="font-semibold text-ink">một đầu mối</strong> cho tour, vé tham quan, di chuyển và lưu trú từ homestay đến resort. Chúng tôi thiết kế hành trình quanh hồ Xuân Hương, Cầu Đất, Langbiang và Tuyền Lâm để bạn cảm nhận đúng nhịp sống cao nguyên.',
                'metaLine' => 'Giấy phép lữ hành số 0045/2024/TCDL-GPLHQT', 'ctaLabel' => 'Về chúng tôi', 'ctaUrl' => '/ve-chung-toi', 'image' => null, 'imageAlt' => 'Hi Đà Lạt'),




'en' => array('key' => 'company_intro', 'eyebrow' => 'Da Lat experts', 'title' => 'Eternal spring on the plateau', 'subtitle' => null,
                'body' => 'Hi Da Lat is a locally built Da Lat travel platform — <strong class="font-semibold text-ink">one place</strong> for tours, tickets, transfers and stays from homestays to resorts. We craft itineraries around Xuan Huong Lake, Cau Dat, Langbiang and Tuyen Lam so you feel the real highland rhythm.',
                'metaLine' => 'Travel license No. 0045/2024/TCDL-GPLHQT', 'ctaLabel' => 'About us', 'ctaUrl' => '/ve-chung-toi', 'image' => null, 'imageAlt' => 'Hi Da Lat'),
        ),
        'featured_tours' => array(
            'vi' => array('key' => 'featured_tours', 'eyebrow' => 'Yêu thích', 'title' => 'Tour được đặt nhiều nhất', 'subtitle' => 'Hành trình khách đánh giá cao trong 12 tháng qua.', 'body' => null, 'metaLine' => null, 'ctaLabel' => null, 'ctaUrl' => null, 'image' => null, 'imageAlt' => null),




'en' => array('key' => 'featured_tours', 'eyebrow' => 'Popular', 'title' => 'Most booked tours', 'subtitle' => 'Top-rated itineraries over the past 12 months.', 'body' => null, 'metaLine' => null, 'ctaLabel' => null, 'ctaUrl' => null, 'image' => null, 'imageAlt' => null),
        ),
        'featured_cruises' => array(
            'vi' => array('key' => 'featured_cruises', 'eyebrow' => 'Hồ Tuyền Lâm', 'title' => 'Thuyền & SUP trên hồ núi', 'subtitle' => 'Hoàng hôn picnic, kayak sáng sớm — trải nghiệm nước đặc trưng Đà Lạt.', 'body' => null, 'metaLine' => null, 'ctaLabel' => null, 'ctaUrl' => null, 'image' => null, 'imageAlt' => null),




'en' => array('key' => 'featured_cruises', 'eyebrow' => 'Tuyen Lam Lake', 'title' => 'Boats & SUP on the mountain lake', 'subtitle' => 'Sunset picnics and dawn paddles — Da Lat\'s đặc trưng water experiences.', 'body' => null, 'metaLine' => null, 'ctaLabel' => null, 'ctaUrl' => null, 'image' => null, 'imageAlt' => null),
        ),
        'featured_trains' => array(
            'vi' => array('key' => 'featured_trains', 'eyebrow' => 'Di chuyển', 'title' => 'Xe khách, limousine & shuttle Đà Lạt', 'subtitle' => 'Từ Sài Gòn, Hà Nội, Nha Trang — một đầu mối đặt vé và đưa đón.', 'body' => null, 'metaLine' => null, 'ctaLabel' => null, 'ctaUrl' => null, 'image' => null, 'imageAlt' => null),




'en' => array('key' => 'featured_trains', 'eyebrow' => 'Getting there', 'title' => 'Buses, limousines & DLI shuttles', 'subtitle' => 'From Sài Gòn, Hanoi, Nha Trang — one place to book transfers.', 'body' => null, 'metaLine' => null, 'ctaLabel' => null, 'ctaUrl' => null, 'image' => null, 'imageAlt' => null),
        ),
        'support_services' => array(
            'vi' => array('key' => 'support_services', 'eyebrow' => 'Dịch vụ', 'title' => 'Vé, trải nghiệm & hỗ trợ', 'subtitle' => 'Canyoning, cà phê, HDV riêng và tiện ích trên cao nguyên.', 'body' => null, 'metaLine' => null, 'ctaLabel' => null, 'ctaUrl' => null, 'image' => null, 'imageAlt' => null),




'en' => array('key' => 'support_services', 'eyebrow' => 'Services', 'title' => 'Tickets, experiences & support', 'subtitle' => 'Canyoning, coffee tours, private guides and highland extras.', 'body' => null, 'metaLine' => null, 'ctaLabel' => null, 'ctaUrl' => null, 'image' => null, 'imageAlt' => null),
        ),
        'destinations' => array(
            'vi' => array('key' => 'destinations', 'eyebrow' => 'Khắp Đà Lạt', 'title' => 'Khu vực được yêu thích', 'subtitle' => 'Trung tâm, Langbiang, Cầu Đất, Tuyền Lâm và combo biển.', 'body' => null, 'metaLine' => null, 'ctaLabel' => null, 'ctaUrl' => null, 'image' => null, 'imageAlt' => null),




'en' => array('key' => 'destinations', 'eyebrow' => 'Across Da Lat', 'title' => 'Favourite areas', 'subtitle' => 'Centre, Langbiang, Cau Dat, Tuyen Lam and beach combos.', 'body' => null, 'metaLine' => null, 'ctaLabel' => null, 'ctaUrl' => null, 'image' => null, 'imageAlt' => null),
        ),
        'testimonials' => array(
            'vi' => array('key' => 'testimonials', 'eyebrow' => 'Khách kể', 'title' => 'Trải nghiệm thật', 'subtitle' => 'Hơn 4.000 khách đã đồng hành cùng Hi Đà Lạt.', 'body' => null, 'metaLine' => null, 'ctaLabel' => 'Xem cảm nhận', 'ctaUrl' => '/cam-nhan-khach-hang', 'image' => null, 'imageAlt' => null),




'en' => array('key' => 'testimonials', 'eyebrow' => 'Guest stories', 'title' => 'Real experiences', 'subtitle' => 'Over 4,000 guests have travelled with Hi Da Lat.', 'body' => null, 'metaLine' => null, 'ctaLabel' => 'All reviews', 'ctaUrl' => '/cam-nhan-khach-hang', 'image' => null, 'imageAlt' => null),
        ),
        'review_platforms' => array(
            'vi' => array('key' => 'review_platforms', 'eyebrow' => null, 'title' => 'Hi Đà Lạt được đánh giá cao trên', 'subtitle' => null, 'body' => null, 'metaLine' => null, 'ctaLabel' => null, 'ctaUrl' => null, 'image' => null, 'imageAlt' => null),




'en' => array('key' => 'review_platforms', 'eyebrow' => null, 'title' => 'Hi Da Lat is highly rated on', 'subtitle' => null, 'body' => null, 'metaLine' => null, 'ctaLabel' => null, 'ctaUrl' => null, 'image' => null, 'imageAlt' => null),
        ),
        'team' => array(
            'vi' => array('key' => 'team', 'eyebrow' => 'Con người Hi Đà Lạt', 'title' => 'Đội ngũ bản địa', 'subtitle' => 'Cùng bạn từ ý tưởng đến khi rời cao nguyên.', 'body' => null, 'metaLine' => null, 'ctaLabel' => 'Gặp đội ngũ', 'ctaUrl' => '/doi-ngu', 'image' => null, 'imageAlt' => null),




'en' => array('key' => 'team', 'eyebrow' => 'The Hi Da Lat team', 'title' => 'Local experts', 'subtitle' => 'With you from the first idea until you leave the highlands.', 'body' => null, 'metaLine' => null, 'ctaLabel' => 'Meet the team', 'ctaUrl' => '/doi-ngu', 'image' => null, 'imageAlt' => null),
        ),
        'videos' => array(
            'vi' => array('key' => 'videos', 'eyebrow' => 'Video', 'title' => 'Đà Lạt qua ống kính', 'subtitle' => 'Sương, hoa và hồ — clip từ khách và đội ngũ.', 'body' => null, 'metaLine' => null, 'ctaLabel' => 'Xem video', 'ctaUrl' => '/video-trai-nghiem', 'image' => null, 'imageAlt' => null),




'en' => array('key' => 'videos', 'eyebrow' => 'Video', 'title' => 'Da Lat on film', 'subtitle' => 'Mist, flowers and lakes — from guests and our team.', 'body' => null, 'metaLine' => null, 'ctaLabel' => 'All videos', 'ctaUrl' => '/video-trai-nghiem', 'image' => null, 'imageAlt' => null),
        ),
        'quick_inquiry' => array(
            'vi' => array('key' => 'quick_inquiry', 'eyebrow' => 'Tư vấn miễn phí', 'title' => 'Gửi lời nhắn', 'subtitle' => null,
                'body' => 'Chưa chọn tour hay khu homestay? Phản hồi trong <strong class="font-semibold text-ink">24 giờ làm việc</strong>, miễn phí.', 'metaLine' => null, 'ctaLabel' => null, 'ctaUrl' => null, 'image' => null, 'imageAlt' => null),




'en' => array('key' => 'quick_inquiry', 'eyebrow' => 'Free advice', 'title' => 'Send a message', 'subtitle' => null,
                'body' => 'Not sure which tour or stay area? Reply within <strong class="font-semibold text-ink">1 business day</strong>, free of charge.', 'metaLine' => null, 'ctaLabel' => null, 'ctaUrl' => null, 'image' => null, 'imageAlt' => null),
        ),
    ),

    'footer_columns' => array(
        array('title' => 'Hi Đà Lạt', 'links' => array(
            array('label' => 'Về chúng tôi', 'route' => array('about')),
            array('label' => 'Cảm nhận khách hàng', 'route' => array('reviews')),
            array('label' => 'Đội ngũ', 'route' => array('team')),
            array('label' => 'Thiết kế tour riêng', 'route' => array('customize')),
        )),
        array('title' => 'Tour phổ biến', 'links' => array(
            array('label' => 'Đà Lạt 3 ngày', 'route' => array('tours.show', array('zone' => 'trung-tam-xuan-huong', 'slug' => 'da-lat-3-ngay-tong-quan'))),
            array('label' => 'Canyoning Datanla', 'route' => array('tours.show', array('zone' => 'datanla-robin-hill', 'slug' => 'canyoning-datanla-1-ngay'))),
            array('label' => 'Cà phê Cầu Đất', 'route' => array('tours.show', array('zone' => 'cau-dat', 'slug' => 'cau-dat-cafe-tea-1-ngay'))),
            array('label' => 'Festival hoa 2 ngày 1 đêm', 'route' => array('tours.show', array('zone' => 'trung-tam-xuan-huong', 'slug' => 'festival-hoa-da-lat-2n1d'))),
        )),
        array('title' => 'Khu vực', 'links' => array(
            array('label' => 'Cầu Đất', 'route' => array('guide.zone', array('zone' => 'cau-dat'))),
            array('label' => 'Langbiang', 'route' => array('guide.zone', array('zone' => 'langbiang-ta-nung'))),
            array('label' => 'Tuyền Lâm', 'route' => array('cruises.index', array('type' => 'thuyen-ho-tuyen-lam'))),
        )),
        array('title' => 'Cẩm nang', 'links' => array(
            array('label' => 'Từ Sài Gòn đi Đà Lạt', 'route' => array('guide.show', array('zone' => 'trung-tam-xuan-huong', 'slug' => 'tu-sai-gon-di-da-lat-the-nao'))),
            array('label' => 'Mùa nào đẹp?', 'route' => array('guide.show', array('zone' => 'trung-tam-xuan-huong', 'slug' => 'da-lat-mua-nao-dep-nhat'))),
            array('label' => 'Ăn gì ở Đà Lạt', 'route' => array('guide.show', array('zone' => 'trung-tam-xuan-huong', 'slug' => 'an-gi-o-da-lat'))),
        )),
    ),

    'footer_seo_links' => array(
        array('label' => 'Cẩm nang Đà Lạt', 'route' => array('guide.zone', array('zone' => 'trung-tam-xuan-huong'))),
        array('label' => 'Tour Đà Lạt trọn gói', 'route' => array('tours.index', array('zone' => 'trung-tam-xuan-huong'))),
        array('label' => 'Thuyền Tuyền Lâm', 'route' => array('cruises.index', array('type' => 'thuyen-ho-tuyen-lam'))),
        array('label' => 'Xe Sài Gòn Đà Lạt', 'route' => array('services.hub', array('cluster' => 'train'))),
        array('label' => 'Thiết kế tour riêng', 'route' => array('customize')),
    ),

    'tour_categories' => array(
        /* ── DANH MỤC (type=region) — khu vực GEO & combo, không chia theo số ngày ── */
        array(
            'zoneSlug' => 'trung-tam-xuan-huong',
            'slug' => 'tour-trung-tam-ho-xuan-huong',
            'type' => 'region',
            'sort' => 0,
            'packageSlugs' => array(
                'da-lat-3-ngay-tong-quan',
                'da-lat-2-ngay-lang-man',
                'city-tour-da-lat-1-ngay',
                'festival-hoa-da-lat-2n1d',
                'gia-dinh-da-lat-2n1d',
                'food-tour-cho-dem-nua-dem',
                'chup-anh-trang-mat-da-lat',
                'nau-an-da-lat-half-day',
            ),
            'name' => array('vi' => 'Tour trung tâm & hồ Xuân Hương', 'en' => 'City centre & Xuan Huong Lake tours'),
            'subtitle' => array('vi' => 'Nhà thờ Con Gà, chợ đêm, Crazy House và vòng hồ Xuân Hương — bán kính đi bộ hoặc 10 phút xe.', 'en' => 'Chicken Church, the night market, Crazy House and the Xuan Huong lakefront — all within walking or a 10-minute drive.'),
            'seo_body' => array('vi' => 'Danh mục theo khu vực: mọi chương trình có điểm chính nằm trong trung tâm Đà Lạt và quanh hồ Xuân Hương, gồm cả tour dài ngày lấy trung tâm làm bản doanh.', 'en' => 'GEO category: every programme centred on downtown Da Lat and Xuan Huong Lake, including longer tours that use the centre as their base.'),
            'faqs' => array(),
        ),
        array(
            'zoneSlug' => 'langbiang-ta-nung',
            'slug' => 'tour-langbiang-ta-nung',
            'type' => 'region',
            'sort' => 1,
            'packageSlugs' => array(
                'da-lat-4-ngay-kham-pha-sau',
                'langbiang-jeep-trekking-1-ngay',
            ),
            'name' => array('vi' => 'Tour Langbiang & Tà Nung', 'en' => 'Langbiang & Ta Nung tours'),
            'subtitle' => array('vi' => 'Đỉnh Langbiang 1.929m, làng K’Ho và cung đường rừng thông Tà Nung phía tây thành phố.', 'en' => 'The 1,929m Langbiang summit, K’Ho villages and the Ta Nung pine-forest road west of the city.'),
            'seo_body' => array('vi' => 'Khu vực cao và mát nhất quanh Đà Lạt — jeep địa hình, trekking đỉnh và văn hoá bản địa K’Ho.', 'en' => 'The highest, coolest area around Da Lat — off-road jeeps, summit treks and K’Ho culture.'),
            'faqs' => array(),
        ),
        array(
            'zoneSlug' => 'tuyen-lam-truc-lam',
            'slug' => 'tour-ho-tuyen-lam-truc-lam',
            'type' => 'region',
            'sort' => 2,
            'packageSlugs' => array(
                'tuyen-lam-truc-lam-robin-1-ngay',
                'da-lat-2-ngay-lang-man',
            ),
            'name' => array('vi' => 'Tour hồ Tuyền Lâm & Trúc Lâm', 'en' => 'Tuyen Lam Lake & Truc Lam tours'),
            'subtitle' => array('vi' => 'Hồ núi lớn nhất Đà Lạt, thiền viện Trúc Lâm và những chuyến thuyền chiều trên mặt hồ.', 'en' => 'Da Lat’s largest mountain lake, the Truc Lam monastery and afternoon boat rides.'),
            'seo_body' => array('vi' => 'Danh mục khu vực hồ Tuyền Lâm — điểm đến yên tĩnh nhất trong bán kính 10km từ trung tâm.', 'en' => 'The Tuyen Lam Lake area — the quietest destination within 10km of the centre.'),
            'faqs' => array(),
        ),
        array(
            'zoneSlug' => 'datanla-robin-hill',
            'slug' => 'tour-datanla-robin-hill',
            'type' => 'region',
            'sort' => 3,
            'packageSlugs' => array(
                'canyoning-datanla-1-ngay',
                'da-lat-3-ngay-tong-quan',
                'gia-dinh-da-lat-2n1d',
                'da-lat-4-ngay-kham-pha-sau',
            ),
            'name' => array('vi' => 'Tour Datanla & Robin Hill', 'en' => 'Datanla & Robin Hill tours'),
            'subtitle' => array('vi' => 'Thác Datanla, máng trượt alpine coaster và cáp treo Robin Hill nhìn xuống hồ Tuyền Lâm.', 'en' => 'Datanla Falls, the alpine coaster and the Robin Hill cable car over Tuyen Lam Lake.'),
            'seo_body' => array('vi' => 'Cụm điểm đến vận động nhiều nhất Đà Lạt — từ máng trượt an toàn cho trẻ tới canyoning nhảy thác.', 'en' => 'Da Lat’s most active cluster — from a kid-safe coaster to full canyoning descents.'),
            'faqs' => array(),
        ),
        array(
            'zoneSlug' => 'cau-dat',
            'slug' => 'tour-cao-nguyen-cau-dat',
            'type' => 'region',
            'sort' => 4,
            'packageSlugs' => array(
                'cau-dat-cafe-tea-1-ngay',
                'da-lat-3-ngay-tong-quan',
                'da-lat-4-ngay-kham-pha-sau',
                'da-lat-nha-trang-3n2d',
            ),
            'name' => array('vi' => 'Tour cao nguyên Cầu Đất', 'en' => 'Cau Dat highlands tours'),
            'subtitle' => array('vi' => 'Trên 1.600m — vườn Arabica specialty, đồi chè trăm tuổi và sương sớm dày mỗi ngày.', 'en' => 'Above 1,600m — specialty Arabica farms, century-old tea hills and daily morning mist.'),
            'seo_body' => array('vi' => 'Vùng nông nghiệp cao nhất Lâm Đồng, cách trung tâm khoảng 25km — nơi đặt tour cà phê và chụp ảnh đồi chè.', 'en' => 'Lam Dong’s highest farming belt, about 25km from the centre — home to coffee tours and tea-hill photography.'),
            'faqs' => array(),
        ),
        array(
            'zoneSlug' => 'ngoai-o-thanh-pho',
            'slug' => 'tour-ngoai-o-thac-nuoc',
            'type' => 'region',
            'sort' => 5,
            'packageSlugs' => array(
                'thac-voi-cu-lan-1-ngay',
                'da-lat-4-ngay-kham-pha-sau',
            ),
            'name' => array('vi' => 'Tour ngoại ô & thác nước', 'en' => 'Outskirts & waterfalls tours'),
            'subtitle' => array('vi' => 'Thác Voi, Đường hầm đất sét và làng Cù Lần — vòng ngoại ô phía bắc và tây bắc.', 'en' => 'Elephant Falls, the Clay Tunnel and Cu Lan village — the northern and northwestern loop.'),
            'seo_body' => array('vi' => 'Danh mục vùng ngoại ô: các điểm cách trung tâm 20–35km, thường ghép trong một ngày đường đèo.', 'en' => 'The outskirts category: sights 20–35km out, usually combined into one mountain-road day.'),
            'faqs' => array(),
        ),
        array(
            'zoneSlug' => 'ket-hop-nha-trang',
            'slug' => 'combo-da-lat-nha-trang',
            'type' => 'region',
            'sort' => 6,
            'packageSlugs' => array(
                'da-lat-nha-trang-3n2d',
            ),
            'name' => array('vi' => 'Combo Đà Lạt — Nha Trang', 'en' => 'Da Lat — Nha Trang combos'),
            'subtitle' => array('vi' => 'Cao nguyên mát và biển Nha Trang trong một hành trình, nối bằng đèo Khánh Vĩnh khoảng 3,5 giờ.', 'en' => 'Cool highlands and the Nha Trang coast in one trip, linked by the 3.5-hour Khanh Vinh pass.'),
            'seo_body' => array('vi' => 'Danh mục combo: hành trình hai điểm đến, có thể đảo chiều xuất phát từ Nha Trang.', 'en' => 'Combo category: a two-destination journey that can also start from Nha Trang.'),
            'faqs' => array(),
        ),
        array(
            'zoneSlug' => 'ket-hop-mui-ne',
            'slug' => 'combo-da-lat-mui-ne',
            'type' => 'region',
            'sort' => 7,
            'packageSlugs' => array(
                'da-lat-mui-ne-3n2d',
            ),
            'name' => array('vi' => 'Combo Đà Lạt — Mũi Né', 'en' => 'Da Lat — Mui Ne combos'),
            'subtitle' => array('vi' => 'Sương cao nguyên buổi sáng, đồi cát Phan Thiết buổi chiều — hai khí hậu trong một chuyến.', 'en' => 'Highland mist in the morning, Phan Thiet dunes in the afternoon — two climates in one trip.'),
            'seo_body' => array('vi' => 'Danh mục combo Đà Lạt — Mũi Né, phổ biến với khách khởi hành từ TP.HCM.', 'en' => 'The Da Lat — Mui Ne combo category, popular with travellers leaving from Ho Chi Minh City.'),
            'faqs' => array(),
        ),

        /* ── CHỦ ĐỀ (A) type=theme — thời lượng chương trình, gắn hub zone ── */
        array(
            'zoneSlug' => 'trung-tam-xuan-huong',
            'slug' => 'tour-trong-ngay',
            'type' => 'theme',
            'sort' => 0,
            'minDays' => 1,
            'maxDays' => 1,
            'packageSlugs' => array(
                'city-tour-da-lat-1-ngay',
                'canyoning-datanla-1-ngay',
                'langbiang-jeep-trekking-1-ngay',
                'cau-dat-cafe-tea-1-ngay',
                'thac-voi-cu-lan-1-ngay',
                'food-tour-cho-dem-nua-dem',
                'tuyen-lam-truc-lam-robin-1-ngay',
                'chup-anh-trang-mat-da-lat',
                'nau-an-da-lat-half-day',
            ),
            'name' => array('vi' => 'Tour trong ngày', 'en' => 'Day tours'),
            'subtitle' => array('vi' => 'Trọn vẹn trong ngày — không qua đêm tại Đà Lạt.', 'en' => 'A full day — no overnight in Da Lat.'),
            'seo_body' => array('vi' => 'Chủ đề theo thời lượng chương trình — khác danh mục theo khu vực / combo.', 'en' => 'Program-duration theme — distinct from GEO/combo category pages.'),
            'faqs' => array(),
        ),
        array(
            'zoneSlug' => 'trung-tam-xuan-huong',
            'slug' => 'tour-2-ngay-1-dem',
            'type' => 'theme',
            'sort' => 1,
            'minDays' => 2,
            'maxDays' => 2,
            'packageSlugs' => array(
                'da-lat-2-ngay-lang-man',
                'festival-hoa-da-lat-2n1d',
                'gia-dinh-da-lat-2n1d',
            ),
            'name' => array('vi' => 'Tour 2 ngày 1 đêm', 'en' => '2 days 1 night'),
            'subtitle' => array('vi' => 'Cuối tuần ngắn — một đêm nghỉ tại Đà Lạt.', 'en' => 'Short weekend — one overnight in Da Lat.'),
            'seo_body' => array('vi' => 'Chủ đề theo thời lượng chương trình — khác danh mục theo khu vực / combo.', 'en' => 'Program-duration theme — distinct from GEO/combo category pages.'),
            'faqs' => array(),
        ),
        array(
            'zoneSlug' => 'trung-tam-xuan-huong',
            'slug' => 'tour-3-ngay-2-dem',
            'type' => 'theme',
            'sort' => 2,
            'minDays' => 3,
            'maxDays' => 3,
            'packageSlugs' => array(
                'da-lat-3-ngay-tong-quan',
                'da-lat-nha-trang-3n2d',
                'da-lat-mui-ne-3n2d',
            ),
            'name' => array('vi' => 'Tour 3 ngày 2 đêm', 'en' => '3 days 2 nights'),
            'subtitle' => array('vi' => 'Khám phá vừa đủ — hai đêm tại Đà Lạt.', 'en' => 'Enough depth — two nights in Da Lat.'),
            'seo_body' => array('vi' => 'Chủ đề theo thời lượng chương trình — khác danh mục theo khu vực / combo.', 'en' => 'Program-duration theme — distinct from GEO/combo category pages.'),
            'faqs' => array(),
        ),
        array(
            'zoneSlug' => 'trung-tam-xuan-huong',
            'slug' => 'tour-4-ngay-3-dem',
            'type' => 'theme',
            'sort' => 3,
            'minDays' => 4,
            'maxDays' => 4,
            'packageSlugs' => array(
                'da-lat-4-ngay-kham-pha-sau',
            ),
            'name' => array('vi' => 'Tour 4 ngày 3 đêm', 'en' => '4 days 3 nights'),
            'subtitle' => array('vi' => 'Khám phá sâu hơn — ba đêm trải nghiệm Đà Lạt.', 'en' => 'Deeper exploration — three nights in Da Lat.'),
            'seo_body' => array('vi' => 'Chủ đề theo thời lượng chương trình — khác danh mục theo khu vực / combo.', 'en' => 'Program-duration theme — distinct from GEO/combo category pages.'),
            'faqs' => array(),
        ),
        array(
            'zoneSlug' => 'trung-tam-xuan-huong',
            'slug' => 'tour-tu-5-ngay',
            'type' => 'theme',
            'sort' => 4,
            'minDays' => 5,
            'maxDays' => null,
            'packageSlugs' => array(),
            'name' => array('vi' => 'Tour từ 5 ngày', 'en' => '5+ day tours'),
            'subtitle' => array('vi' => 'Tour dài ngày và combo nhiều điểm đến từ Đà Lạt.', 'en' => 'Extended tours and multi-stop combos from Da Lat.'),
            'seo_body' => array('vi' => 'Chủ đề theo thời lượng chương trình — khác danh mục theo khu vực / combo.', 'en' => 'Program-duration theme — distinct from GEO/combo category pages.'),
            'faqs' => array(),
        ),

        /* ── CHỦ ĐỀ (B) type=theme — tính chất / insight, KHÔNG clone tên zone GEO ── */
        array(
            'zoneSlug' => 'trung-tam-xuan-huong',
            'slug' => 'cafe-nong-trai',
            'type' => 'theme',
            'sort' => 10,
            'packageSlugs' => array(
                'da-lat-3-ngay-tong-quan',
                'da-lat-4-ngay-kham-pha-sau',
                'cau-dat-cafe-tea-1-ngay',
                'da-lat-nha-trang-3n2d',
            ),
            'name' => array('vi' => 'Cà phê & nông trại', 'en' => 'Coffee & farms'),
            'subtitle' => array('vi' => 'Cupping Arabica tại vườn, đồi chè, vườn dâu và trại bò sữa cao nguyên.', 'en' => 'Arabica cupping on the farm, tea hills, strawberry gardens and highland dairies.'),
            'seo_body' => array('vi' => 'Chủ đề theo tính chất chương trình: mọi tour có phần nông nghiệp — không trùng danh mục khu vực Cầu Đất.', 'en' => 'Programme-character theme covering every tour with a farm component — not the Cau Dat GEO category.'),
            'faqs' => array(),
        ),
        array(
            'zoneSlug' => 'trung-tam-xuan-huong',
            'slug' => 'phieu-luu-canyon',
            'type' => 'theme',
            'sort' => 11,
            'packageSlugs' => array(
                'da-lat-4-ngay-kham-pha-sau',
                'canyoning-datanla-1-ngay',
                'langbiang-jeep-trekking-1-ngay',
            ),
            'name' => array('vi' => 'Phiêu lưu & canyoning', 'en' => 'Adventure & canyoning'),
            'subtitle' => array('vi' => 'Nhảy thác, đu dây, jeep địa hình và trekking đỉnh — có huấn luyện viên và thiết bị đạt chuẩn.', 'en' => 'Waterfall jumps, abseiling, off-road jeeps and summit treks — certified guides and gear.'),
            'seo_body' => array('vi' => 'Phân khúc khách thích vận động mạnh, lọc theo tính chất hoạt động thay vì theo vùng.', 'en' => 'The high-adrenaline segment, filtered by activity type rather than by area.'),
            'faqs' => array(),
        ),
        array(
            'zoneSlug' => 'trung-tam-xuan-huong',
            'slug' => 'cuoi-tuan-sai-gon',
            'type' => 'theme',
            'sort' => 12,
            'packageSlugs' => array(
                'da-lat-3-ngay-tong-quan',
                'da-lat-2-ngay-lang-man',
                'festival-hoa-da-lat-2n1d',
                'gia-dinh-da-lat-2n1d',
                'da-lat-mui-ne-3n2d',
            ),
            'name' => array('vi' => 'Cuối tuần từ Sài Gòn', 'en' => 'Weekend from Saigon'),
            'subtitle' => array('vi' => 'Limousine đêm hoặc bay 50 phút — lịch 2N1D và 3N2D vừa khít hai ngày nghỉ.', 'en' => 'Overnight limousine or a 50-minute flight — 2N1D and 3N2D itineraries that fit a weekend.'),
            'seo_body' => array('vi' => 'Chủ đề theo thị trường nguồn: khách TP.HCM thoát nóng cuối tuần, không phải trang danh mục vùng.', 'en' => 'Source-market theme for Ho Chi Minh City weekenders — not a GEO category page.'),
            'faqs' => array(),
        ),
        array(
            'zoneSlug' => 'trung-tam-xuan-huong',
            'slug' => 'gia-dinh-tre-em',
            'type' => 'theme',
            'sort' => 13,
            'packageSlugs' => array(
                'city-tour-da-lat-1-ngay',
                'thac-voi-cu-lan-1-ngay',
                'gia-dinh-da-lat-2n1d',
                'tuyen-lam-truc-lam-robin-1-ngay',
            ),
            'name' => array('vi' => 'Gia đình có trẻ em', 'en' => 'Family with kids'),
            'subtitle' => array('vi' => 'Lịch nhẹ, xe riêng, điểm dừng có nhà vệ sinh và hoạt động an toàn cho trẻ dưới 10 tuổi.', 'en' => 'Light pacing, private car, stops with facilities and activities safe for under-10s.'),
            'seo_body' => array('vi' => 'Phân khúc gia đình — lọc theo nhu cầu khách, dùng chung tour với các danh mục khu vực.', 'en' => 'The family segment — a needs-based filter that shares tours with the GEO categories.'),
            'faqs' => array(),
        ),
        array(
            'zoneSlug' => 'trung-tam-xuan-huong',
            'slug' => 'cap-doi-lang-man',
            'type' => 'theme',
            'sort' => 14,
            'packageSlugs' => array(
                'da-lat-2-ngay-lang-man',
                'tuyen-lam-truc-lam-robin-1-ngay',
                'chup-anh-trang-mat-da-lat',
            ),
            'name' => array('vi' => 'Cặp đôi & lãng mạn', 'en' => 'Couples & romance'),
            'subtitle' => array('vi' => 'Trăng mật mini, chụp ảnh pre-wedding và hoàng hôn trên hồ — nhóm riêng hai người.', 'en' => 'Mini honeymoons, pre-wedding shoots and lake sunsets — private for two.'),
            'seo_body' => array('vi' => 'Chủ đề phân khúc cặp đôi; điểm đến cụ thể vẫn thuộc các danh mục khu vực tương ứng.', 'en' => 'A couples-segment theme; the destinations themselves stay in their GEO categories.'),
            'faqs' => array(),
        ),
        array(
            'zoneSlug' => 'trung-tam-xuan-huong',
            'slug' => 'festival-hoa',
            'type' => 'theme',
            'sort' => 15,
            'packageSlugs' => array(
                'festival-hoa-da-lat-2n1d',
                'chup-anh-trang-mat-da-lat',
            ),
            'name' => array('vi' => 'Mùa hoa & festival', 'en' => 'Flower seasons & festival'),
            'subtitle' => array('vi' => 'Festival hoa cuối tháng 12, mai anh đào tháng 1–2 và hướng dương tháng 11 — nên đặt trước 2–4 tuần.', 'en' => 'The late-December flower festival, cherry blossom in Jan–Feb and sunflowers in November — book 2–4 weeks ahead.'),
            'seo_body' => array('vi' => 'Insight theo mùa vụ, không gắn với một khu vực cụ thể nào của Đà Lạt.', 'en' => 'A seasonal insight that is not tied to any single Da Lat area.'),
            'faqs' => array(),
        ),
        array(
            'zoneSlug' => 'trung-tam-xuan-huong',
            'slug' => 'am-thuc-cao-nguyen',
            'type' => 'theme',
            'sort' => 16,
            'packageSlugs' => array(
                'food-tour-cho-dem-nua-dem',
                'nau-an-da-lat-half-day',
            ),
            'name' => array('vi' => 'Ẩm thực cao nguyên', 'en' => 'Highland food'),
            'subtitle' => array('vi' => 'Chợ đêm, bánh căn, atiso và lớp nấu ăn cùng đầu bếp địa phương.', 'en' => 'The night market, banh can, artichoke dishes and cooking classes with local chefs.'),
            'seo_body' => array('vi' => 'Chủ đề foodie — gom tour ăn uống và lớp học nấu ăn, không tạo thêm trang vùng.', 'en' => 'A foodie theme grouping tasting tours and cooking classes without adding a GEO page.'),
            'faqs' => array(),
        ),
    ),

    'listing_faqs' => array(
        array('q' => 'Tour có bao gồm khách sạn không?', 'a' => 'Hầu hết tour chưa gồm lưu trú trong giá. Bạn có thể đặt homestay, villa hoặc resort qua :brand — tư vấn viên gợi ý theo khu và ngân sách.'),
        array('q' => 'Đà Lạt lạnh không, cần mang gì?', 'a' => 'Nhiệt độ thường 15–24°C, tối có thể 10–12°C — áo khoác mỏng, giày trek nếu đi Langbiang/canyoning.'),
        array('q' => 'Bay DLI hay đi xe từ Sài Gòn?', 'a' => 'Xe giường/limousine 5–7h, rẻ hơn; bay ~1h + shuttle 45 phút — tiện hơn nếu ít ngày.'),
        array('q' => 'Canyoning hủy khi mưa?', 'a' => 'Đúng — đổi ngày miễn phí hoặc hoàn theo chính sách khi mưa lớn không an toàn.'),
    ),

    'duration_buckets' => array(
        'half-day' => 'Nửa ngày',
        '1-day' => '1 ngày',
        '2n1d' => '2 ngày 1 đêm',
        '3n2d' => '3 ngày 2 đêm',
        '4n3d' => '4 ngày 3 đêm',
        '5-plus-days' => 'Từ 5 ngày',
    ),

    'travel_style_labels' => array(
        'day-trip' => 'Tour trong ngày',
        '2n1d' => '2 ngày 1 đêm',
        '3n2d' => '3 ngày 2 đêm',
        '4n3d' => '4 ngày 3 đêm',
        '5-plus-days' => 'Từ 5 ngày',
        'cafe-nong-trai' => 'Cà phê & nông trại',
        'phieu-luu-canyon' => 'Phiêu lưu & canyoning',
        'cuoi-tuan-sai-gon' => 'Cuối tuần từ Sài Gòn',
        'gia-dinh-tre-em' => 'Gia đình có trẻ em',
        'cap-doi-lang-man' => 'Cặp đôi & lãng mạn',
        'festival-hoa' => 'Mùa hoa & festival',
        'am-thuc-cao-nguyen' => 'Ẩm thực cao nguyên',
    ),
);

$__servicesSeed = [
    'service_clusters' => [
        ['code' => 'train', 'nav_label' => 'Di chuyển', 'label' => 'Xe khách, limousine & xe liên tỉnh', 'icon' => 'train', 'hub_key' => 'trains_hub', 'sort' => 1],
        ['code' => 'flight', 'nav_label' => 'Máy bay & đưa đón', 'label' => 'Vé bay DLI & shuttle Liên Khương', 'icon' => 'plane', 'hub_key' => 'flights_hub', 'sort' => 2],
        ['code' => 'stay', 'nav_label' => 'Lưu trú', 'label' => 'Homestay, villa & resort Đà Lạt', 'icon' => 'building', 'hub_key' => 'stays_hub', 'sort' => 3],
        ['code' => 'experience', 'nav_label' => 'Vui chơi', 'label' => 'Vé & tour trải nghiệm Đà Lạt', 'icon' => 'sparkles', 'hub_key' => 'experiences_hub', 'sort' => 4],
        ['code' => 'other', 'nav_label' => 'Dịch vụ', 'label' => 'Hỗ trợ & tiện ích trên cao nguyên', 'icon' => 'briefcase', 'hub_key' => 'extras_hub', 'sort' => 5],
    ],
    'service_categories' => [
        ['cluster' => 'train', 'slug' => 'xe-giuong-sgn-da-lat', 'name' => 'Xe giường Sài Gòn ↔ Đà Lạt', 'sort' => 1, 'intro' => 'Tuyến chủ lực cuối tuần — khởi hành tối, tới sáng (~5–7h).', 'seo_body' => 'Khách TP.HCM chiếm phần lớn lượt khách Đà Lạt. :brand đặt ghế giường nằm hoặc ghế ngồi limousine.'],
        ['cluster' => 'train', 'slug' => 'xe-giuong-ha-noi-da-lat', 'name' => 'Xe giường Hà Nội ↔ Đà Lạt', 'sort' => 2, 'intro' => 'Xe đêm xuyên miền — ~30–36h.', 'seo_body' => 'Nên cân nhắc bay HAN—DLI nếu thiếu thời gian.'],
        ['cluster' => 'train', 'slug' => 'limousine-sai-gon-da-lat', 'name' => 'Limousine Sài Gòn ↔ Đà Lạt', 'sort' => 3, 'intro' => 'Xe 9–16 chỗ, ~5–6h ban ngày.', 'seo_body' => 'Phổ biến gia đình — đặt trước thứ 6.'],
        ['cluster' => 'train', 'slug' => 'xe-lien-tinh-bien-cao-nguyen', 'name' => 'Xe Nha Trang / Mũi Né / BMT ↔ Đà Lạt', 'sort' => 4, 'intro' => 'Kết nối biển và Tây Nguyên.', 'seo_body' => 'Nền tảng combo Đà Lạt—Nha Trang / Mũi Né.'],
        ['cluster' => 'train', 'slug' => 'xe-rieng-charter', 'name' => 'Xe riêng & charter liên tỉnh', 'sort' => 5, 'intro' => 'Ô tô 4–16 chỗ theo ngày.', 'seo_body' => 'Phù hợp đoàn hoặc dừng chụp đèo.'],
        ['cluster' => 'train', 'slug' => 'shuttle-san-bay-dli', 'name' => 'Shuttle sân bay Liên Khương (DLI)', 'sort' => 6, 'intro' => 'Sân bay ~30km — shuttle ~45 phút.', 'seo_body' => 'Gộp vé bay qua :brand.'],
        ['cluster' => 'train', 'slug' => 'xe-noi-thanh-da-lat', 'name' => 'Xe đón bến & nội thành', 'sort' => 7, 'intro' => 'Đón bến xe/sân bay trong Đà Lạt.', 'seo_body' => 'Tiện khi mang vali hoặc đi cùng trẻ nhỏ.'],
        ['cluster' => 'flight', 'slug' => 'noi-dia-dli', 'name' => 'Vé nội địa tới Liên Khương (DLI)', 'sort' => 1, 'intro' => 'Bay Sài Gòn/Hà Nội/DAD.', 'seo_body' => 'SGN—DLI ~50 phút.'],
        ['cluster' => 'flight', 'slug' => 'dua-don-dli', 'name' => 'Đưa đón sân bay DLI ↔ thành phố', 'sort' => 2, 'intro' => 'Door-to-city.', 'seo_body' => ':brand theo dõi chuyến bay.'],
        ['cluster' => 'flight', 'slug' => 'combo-bay-shuttle', 'name' => 'Combo vé bay + shuttle', 'sort' => 3, 'intro' => 'Một báo giá bay + xe.', 'seo_body' => 'Tiện khách lần đầu.'],
        ['cluster' => 'experience', 'slug' => 'city-tour-da-lat', 'name' => 'City tour & điểm biểu tượng', 'sort' => 1, 'intro' => 'Crazy House, Linh Phước, hồ Xuân Hương.', 'seo_body' => 'Must-do lần đầu.'],
        ['cluster' => 'experience', 'slug' => 'phieu-luu-datanla-langbiang', 'name' => 'Canyoning, Datanla & Langbiang', 'sort' => 2, 'intro' => 'HLV chứng nhận.', 'seo_body' => 'Top activity quốc tế.'],
        ['cluster' => 'experience', 'slug' => 'cafe-agri-cau-dat', 'name' => 'Cà phê, chè & nông trại', 'sort' => 3, 'intro' => 'Cầu Đất agri-tourism.', 'seo_body' => 'Specialty coffee must-do.'],
        ['cluster' => 'experience', 'slug' => 'hoa-linh-thien', 'name' => 'Hoa, chùa & check-in', 'sort' => 4, 'intro' => 'Vườn hoa, Lumiere, Linh Phước.', 'seo_body' => 'Festival cuối tháng 12.'],
        ['cluster' => 'experience', 'slug' => 'ngoai-o-thac-lang', 'name' => 'Ngoại ô: thác, hầm, Cù Lần', 'sort' => 5, 'intro' => 'Thác Voi, Clay Tunnel.', 'seo_body' => 'Phù hợp gia đình.'],
        ['cluster' => 'experience', 'slug' => 'am-thuc-dem', 'name' => 'Ẩm thực & chợ đêm', 'sort' => 6, 'intro' => 'Food tour, cooking class.', 'seo_body' => 'Chợ đêm không thể bỏ qua.'],
        ['cluster' => 'experience', 'slug' => 'tuyen-lam-cap-treo', 'name' => 'Tuyền Lâm & cáp treo', 'sort' => 7, 'intro' => 'Robin Hill, Trúc Lâm.', 'seo_body' => 'Yên tĩnh cho cặp đôi.'],
        ['cluster' => 'other', 'slug' => 'huong-dan-rieng', 'name' => 'HDV riêng & wedding photo', 'sort' => 1, 'intro' => 'Guide tiếng Anh, pre-wedding.'],
        ['cluster' => 'other', 'slug' => 'xe-tien-ich', 'name' => 'Xe có lái, thuê xe, gửi đồ', 'sort' => 2, 'intro' => 'Xe máy, ô tô có tài.'],
        ['cluster' => 'other', 'slug' => 'ho-tro-dac-biet', 'name' => 'Hoa surprise & khẩn cấp 24/7', 'sort' => 3, 'intro' => 'Setup hoa, hotline.'],
    ],
    'services' => [
        // TRAIN (10)
        ['code' => 'train-sleeper-sgn-dl-oneway', 'cluster' => 'train', 'category_slug' => 'xe-giuong-sgn-da-lat', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Xe giường nằm Sài Gòn → Đà Lạt (một chiều)', 'slug' => 'xe-giuong-sai-gon-da-lat-mot-chieu',
            'price_from' => 280000, 'currency' => 'VND', 'rating' => 4.5, 'review_count' => 892,
            'is_featured' => true, 'location_label' => 'Sài Gòn → Đà Lạt',
            'summary' => 'Khởi hành tối (~19h–22h), tới sáng — tiết kiệm 1 đêm lưu trú.',
            'highlights' => ['5–7 giờ', 'Giường nằm hoặc ghế ngả', 'Đón bến Miền Đông / quận trung tâm'],
            'inclusions' => ['Vé một chiều'], 'exclusions' => ['Đón tận nhà xa trung tâm'],
            'notes' => ['Giá theo hãng xe — chốt khi đặt.'], 'attrs' => ['from' => 'TP.HCM', 'to' => 'Đà Lạt', 'duration_hours' => 6, 'vehicle_type' => 'xe giường nằm']],
        ['code' => 'train-sleeper-sgn-dl-round', 'cluster' => 'train', 'category_slug' => 'xe-giuong-sgn-da-lat', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Xe giường khứ hồi Sài Gòn ↔ Đà Lạt', 'slug' => 'xe-giuong-khu-hoi-sai-gon-da-lat',
            'price_from' => 520000, 'currency' => 'VND', 'rating' => 4.6, 'review_count' => 456,
            'is_featured' => true, 'is_hot_deal' => true, 'location_label' => 'Sài Gòn ↔ Đà Lạt',
            'summary' => 'Gói cuối tuần — đi tối thứ 6, về tối chủ nhật.',
            'highlights' => ['Rẻ hơn 2 chiều lẻ', 'Giữ chỗ chiều về'], 'inclusions' => ['2 chiều'], 'exclusions' => [],
            'notes' => [], 'attrs' => ['from' => 'TP.HCM', 'to' => 'Đà Lạt', 'vehicle_type' => 'xe giường nằm']],
        ['code' => 'train-sleeper-hn-dl', 'cluster' => 'train', 'category_slug' => 'xe-giuong-ha-noi-da-lat', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Xe giường Hà Nội → Đà Lạt', 'slug' => 'xe-giuong-ha-noi-da-lat',
            'price_from' => 550000, 'currency' => 'VND', 'rating' => 4.4, 'review_count' => 234,
            'is_featured' => false, 'location_label' => 'Hà Nội → Đà Lạt',
            'summary' => 'Xe đêm xuyên miền ~30–36h — hoặc cân nhắc bay Hà Nội—DLI.',
            'highlights' => ['Giá thấp', 'Ghế ngả/giường'], 'inclusions' => ['Vé một chiều'], 'exclusions' => ['Ăn trên xe'],
            'notes' => ['Tuyến dài — mang snack và áo ấm.'], 'attrs' => ['from' => 'Hà Nội', 'to' => 'Đà Lạt', 'duration_hours' => 32]],
        ['code' => 'train-limousine-sgn-dl', 'cluster' => 'train', 'category_slug' => 'limousine-sai-gon-da-lat', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Limousine Sài Gòn → Đà Lạt', 'slug' => 'limousine-sai-gon-da-lat',
            'price_from' => 320000, 'currency' => 'VND', 'rating' => 4.7, 'review_count' => 678,
            'is_featured' => true, 'location_label' => 'TP.HCM → Đà Lạt',
            'summary' => '9–16 chỗ, đón trung tâm, ~5–6h ban ngày.',
            'highlights' => ['Đón quận 1/3', 'Wifi', 'Ghế massage'], 'inclusions' => ['Limousine một chiều'], 'exclusions' => ['Phụ phí đón xa'],
            'notes' => ['Đặt trước cuối tuần.'], 'attrs' => ['from' => 'TP.HCM', 'to' => 'Đà Lạt', 'duration_hours' => 5.5, 'vehicle_type' => 'limousine']],
        ['code' => 'train-bus-nhatrang-dl', 'cluster' => 'train', 'category_slug' => 'xe-lien-tinh-bien-cao-nguyen', 'zone_slug' => 'ket-hop-nha-trang',
            'title' => 'Xe khách Nha Trang → Đà Lạt', 'slug' => 'xe-khach-nha-trang-da-lat',
            'price_from' => 180000, 'currency' => 'VND', 'rating' => 4.5, 'review_count' => 312,
            'is_featured' => true, 'location_label' => 'Nha Trang → Đà Lạt',
            'summary' => 'Qua đèo Khánh Vĩnh ~3.5–4h — nền tảng combo biển + cao nguyên.',
            'highlights' => ['Nhiều chuyến/ngày', 'Ghế ngồi/limousine'], 'inclusions' => ['Vé một chiều'], 'exclusions' => [],
            'notes' => ['Đường đèo — dễ say xe.'], 'attrs' => ['from' => 'Nha Trang', 'to' => 'Đà Lạt', 'duration_hours' => 4]],
        ['code' => 'train-bus-muine-dl', 'cluster' => 'train', 'category_slug' => 'xe-lien-tinh-bien-cao-nguyen', 'zone_slug' => 'ket-hop-mui-ne',
            'title' => 'Xe khách Mũi Né → Đà Lạt', 'slug' => 'xe-khach-mui-ne-da-lat',
            'price_from' => 200000, 'currency' => 'VND', 'rating' => 4.4, 'review_count' => 145,
            'is_featured' => false, 'location_label' => 'Mũi Né → Đà Lạt',
            'summary' => 'Đèo Ngoạn Mục ~4h — combo sa mạc mini + cao nguyên.',
            'highlights' => ['Open bus / xe khách'], 'inclusions' => ['Vé một chiều'], 'exclusions' => [],
            'notes' => [], 'attrs' => ['from' => 'Mũi Né', 'to' => 'Đà Lạt', 'duration_hours' => 4]],
        ['code' => 'train-bus-bmt-dl', 'cluster' => 'train', 'category_slug' => 'xe-lien-tinh-bien-cao-nguyen', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Xe Buôn Ma Thuột → Đà Lạt', 'slug' => 'xe-buon-ma-thuot-da-lat',
            'price_from' => 120000, 'currency' => 'VND', 'rating' => 4.5, 'review_count' => 98,
            'is_featured' => false, 'location_label' => 'BMT → Đà Lạt',
            'summary' => '~3h qua Lâm Hà — kết nối Tây Nguyên.',
            'highlights' => ['Tần suất cao'], 'inclusions' => ['Vé một chiều'], 'exclusions' => [],
            'notes' => [], 'attrs' => ['from' => 'Buôn Ma Thuột', 'to' => 'Đà Lạt', 'duration_hours' => 3]],
        ['code' => 'train-private-charter', 'cluster' => 'train', 'category_slug' => 'xe-rieng-charter', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Xe riêng charter liên tỉnh (4–16 chỗ)', 'slug' => 'xe-rieng-charter-lien-tinh',
            'price_from' => 3500000, 'currency' => 'VND', 'rating' => 4.9, 'review_count' => 67,
            'is_featured' => true, 'location_label' => 'Theo tuyến',
            'summary' => 'SGN/NT/MN—Đà Lạt một chiều hoặc theo ngày — dừng đèo tự do.',
            'highlights' => ['Lịch linh hoạt', 'Phù hợp đoàn'], 'inclusions' => ['Xe + tài xế'], 'exclusions' => ['Xăng ngoài km', 'Phí cầu đường'],
            'notes' => ['Báo giá theo tuyến và loại xe.'], 'attrs' => ['service_type' => 'private_charter', 'vehicle_type' => '4–16 chỗ']],
        ['code' => 'train-shuttle-dli', 'cluster' => 'train', 'category_slug' => 'shuttle-san-bay-dli', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Shuttle sân bay Liên Khương ↔ trung tâm Đà Lạt', 'slug' => 'shuttle-san-bay-lien-khuong',
            'price_from' => 120000, 'currency' => 'VND', 'rating' => 4.7, 'review_count' => 534,
            'is_featured' => true, 'location_label' => 'DLI ↔ Đà Lạt',
            'summary' => '~30km, ~45 phút — đón sảnh đến, trả homestay/khách sạn.',
            'highlights' => ['Theo dõi chuyến bay', 'Ghép hoặc riêng'], 'inclusions' => ['Shuttle một chiều'], 'exclusions' => ['Phụ phí đêm khuya'],
            'notes' => ['Gửi số hiệu chuyến bay trước 24h.'], 'attrs' => ['from' => 'DLI', 'to' => 'Đà Lạt', 'duration_hours' => 0.75]],
        ['code' => 'train-city-transfer', 'cluster' => 'train', 'category_slug' => 'xe-noi-thanh-da-lat', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Xe đón bến xe / nội thành Đà Lạt', 'slug' => 'xe-don-ben-noi-thanh-da-lat',
            'price_from' => 150000, 'currency' => 'VND', 'rating' => 4.6, 'review_count' => 289,
            'is_featured' => false, 'location_label' => 'Trong Đà Lạt',
            'summary' => 'Đón bến xe phía Nam/Bắc, đưa homestay hoặc điểm tour.',
            'highlights' => ['Bảng tên đón', '4–7 chỗ'], 'inclusions' => ['Xe một chiều'], 'exclusions' => [],
            'notes' => [], 'attrs' => ['service_type' => 'city_transfer']],

        // FLIGHT (4)
        ['code' => 'flight-sgn-dli', 'cluster' => 'flight', 'category_slug' => 'noi-dia-dli', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Vé máy bay Sài Gòn — Liên Khương (DLI)', 'slug' => 've-may-bay-sgn-dli',
            'price_from' => 890000, 'currency' => 'VND', 'rating' => 4.6, 'review_count' => 567,
            'is_featured' => true, 'location_label' => 'Sài Gòn → Đà Lạt',
            'summary' => 'Bay ~50 phút — phổ biến khách cuối tuần.',
            'highlights' => ['Nhiều chuyến/ngày', 'Gộp shuttle'], 'inclusions' => ['Vé economy', 'Thuế phí'], 'exclusions' => ['Ra thành phố'],
            'notes' => [], 'attrs' => ['from' => 'SGN', 'to' => 'DLI', 'flight_time' => '50m']],
        ['code' => 'flight-han-dli', 'cluster' => 'flight', 'category_slug' => 'noi-dia-dli', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Vé máy bay Hà Nội — Liên Khương (DLI)', 'slug' => 've-may-bay-han-dli',
            'price_from' => 1150000, 'currency' => 'VND', 'rating' => 4.5, 'review_count' => 234,
            'is_featured' => true, 'location_label' => 'Hà Nội → DLI',
            'summary' => 'Bay ~1h40 — nhanh hơn xe đêm 30+ giờ.',
            'highlights' => ['Bay thẳng', 'Gộp shuttle'], 'inclusions' => ['Vé economy'], 'exclusions' => ['Hành lý thêm'],
            'notes' => [], 'attrs' => ['from' => 'HAN', 'to' => 'DLI', 'flight_time' => '1h40m']],
        ['code' => 'flight-transfer-dli', 'cluster' => 'flight', 'category_slug' => 'dua-don-dli', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Đưa đón sân bay DLI — door to hotel', 'slug' => 'dua-don-dli-door-to-hotel',
            'price_from' => 280000, 'currency' => 'VND', 'rating' => 4.8, 'review_count' => 198,
            'is_featured' => true, 'location_label' => 'DLI → Đà Lạt',
            'summary' => 'Xe riêng đón sảnh, trả tận homestay — canh giờ chuyến bay.',
            'highlights' => ['Theo dõi flight', 'Hỗ trợ vali'], 'inclusions' => ['Xe 4 chỗ'], 'exclusions' => ['Vé bay'],
            'notes' => [], 'attrs' => ['from' => 'DLI', 'to' => 'Đà Lạt', 'duration_hours' => 0.75]],
        ['code' => 'flight-combo-fly-shuttle', 'cluster' => 'flight', 'category_slug' => 'combo-bay-shuttle', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Combo vé bay + shuttle Đà Lạt', 'slug' => 'combo-ve-bay-shuttle-da-lat',
            'price_from' => 1150000, 'currency' => 'VND', 'rating' => 4.7, 'review_count' => 89,
            'is_featured' => true, 'is_hot_deal' => true, 'location_label' => 'SGN/Hà Nội → thành phố',
            'summary' => 'Một báo giá bay + xe ra thành phố — không tự ghép.',
            'highlights' => ['Một đầu mối', 'Đổi ngày linh hoạt'], 'inclusions' => ['Vé bay', 'Shuttle'], 'exclusions' => ['Lưu trú', 'Tour'],
            'notes' => ['Giá theo ngày bay.'], 'attrs' => ['from' => 'SGN / HAN', 'to' => 'Đà Lạt']],

        // EXPERIENCE (18)
        ['code' => 'exp-city-tour', 'cluster' => 'experience', 'category_slug' => 'city-tour-da-lat', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Tour thành phố Đà Lạt trọn ngày', 'slug' => 'city-tour-da-lat-tron-ngay',
            'price_from' => 550000, 'currency' => 'VND', 'rating' => 4.8, 'review_count' => 412,
            'is_featured' => true, 'location_label' => 'Trung tâm Đà Lạt',
            'summary' => 'Crazy House, Linh Phước, Domaine de Marie, hồ Xuân Hương.',
            'highlights' => ['08:30–17:00', 'HDV', 'Ghép hoặc riêng'], 'inclusions' => ['Xe', 'Vé điểm', 'HDV'], 'exclusions' => ['Ăn'],
            'notes' => [], 'attrs' => ['duration_hours' => 8, 'activity' => 'city_tour']],
        ['code' => 'exp-canyoning', 'cluster' => 'experience', 'category_slug' => 'phieu-luu-datanla-langbiang', 'zone_slug' => 'datanla-robin-hill',
            'title' => 'Canyoning thác Datanla', 'slug' => 'canyoning-thac-datanla',
            'price_from' => 1200000, 'currency' => 'VND', 'rating' => 4.9, 'review_count' => 356,
            'is_featured' => true, 'location_label' => 'Thác Datanla',
            'summary' => 'HLV quốc tế, nhảy thác, zipline — 3–4 giờ.',
            'highlights' => ['HLV chứng nhận', 'Thiết bị đầy đủ'], 'inclusions' => ['HLV', 'Thiết bị', 'Bảo hiểm cơ bản'], 'exclusions' => ['GoPro'],
            'notes' => ['Biết bơi; từ 16 tuổi.'], 'attrs' => ['duration_hours' => 4, 'activity' => 'canyoning']],
        ['code' => 'exp-langbiang-jeep', 'cluster' => 'experience', 'category_slug' => 'phieu-luu-datanla-langbiang', 'zone_slug' => 'langbiang-ta-nung',
            'title' => 'Jeep địa hình Langbiang', 'slug' => 'jeep-dia-hinh-langbiang',
            'price_from' => 450000, 'currency' => 'VND', 'rating' => 4.7, 'review_count' => 234,
            'is_featured' => true, 'location_label' => 'Núi Langbiang',
            'summary' => 'Jeep lên đồi — view toàn cảnh thành phố trong sương.',
            'highlights' => ['Jeep địa hình', 'View đỉnh'], 'inclusions' => ['Jeep', 'Vé khu'], 'exclusions' => ['Trek chân lên đỉnh'],
            'notes' => [], 'attrs' => ['duration_hours' => 3, 'activity' => 'jeep']],
        ['code' => 'exp-langbiang-trek', 'cluster' => 'experience', 'category_slug' => 'phieu-luu-datanla-langbiang', 'zone_slug' => 'langbiang-ta-nung',
            'title' => 'Trekking đỉnh Langbiang', 'slug' => 'trekking-dinh-langbiang',
            'price_from' => 550000, 'currency' => 'VND', 'rating' => 4.8, 'review_count' => 167,
            'is_featured' => false, 'location_label' => 'Langbiang 2.167m',
            'summary' => 'Trek 2–3h với HDV — đỉnh cao nhất gần thành phố.',
            'highlights' => ['HDV trek', 'Picnic'], 'inclusions' => ['HDV', 'Vé', 'Nước'], 'exclusions' => ['Giày trek'],
            'notes' => ['Mang áo ấm.'], 'attrs' => ['duration_hours' => 5, 'activity' => 'trekking']],
        ['code' => 'exp-datanla-coaster', 'cluster' => 'experience', 'category_slug' => 'phieu-luu-datanla-langbiang', 'zone_slug' => 'datanla-robin-hill',
            'title' => 'Thác Datanla + máng trượt alpine coaster', 'slug' => 'thac-datanla-mang-truot',
            'price_from' => 250000, 'currency' => 'VND', 'rating' => 4.7, 'review_count' => 523,
            'is_featured' => true, 'location_label' => 'Datanla',
            'summary' => 'Vé thác + coaster — phù hợp gia đình.',
            'highlights' => ['Coaster an toàn', 'Chụp thác'], 'inclusions' => ['Vé combo'], 'exclusions' => ['Xe đón'],
            'notes' => [], 'attrs' => ['duration_hours' => 2, 'activity' => 'coaster']],
        ['code' => 'exp-linh-phuoc', 'cluster' => 'experience', 'category_slug' => 'hoa-linh-thien', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Chùa Linh Phước (ve chai) — vé & đưa đón', 'slug' => 'chua-linh-phuoc-ve-dua-don',
            'price_from' => 180000, 'currency' => 'VND', 'rating' => 4.8, 'review_count' => 289,
            'is_featured' => true, 'location_label' => 'Chùa Linh Phước',
            'summary' => 'Chùa ve chai nổi tiếng — ghép city tour hoặc riêng.',
            'highlights' => ['Kiến trúc ve chai', 'Gần ga xe lửa'], 'inclusions' => ['Vé', 'Xe ghép'], 'exclusions' => [],
            'notes' => [], 'attrs' => ['duration_hours' => 1.5, 'activity' => 'temple']],
        ['code' => 'exp-crazy-house', 'cluster' => 'experience', 'category_slug' => 'city-tour-da-lat', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Crazy House (Hang Nga) — vé tham quan', 'slug' => 'crazy-house-ve-tham-quan',
            'price_from' => 60000, 'currency' => 'VND', 'rating' => 4.6, 'review_count' => 678,
            'is_featured' => true, 'location_label' => 'Crazy House',
            'summary' => 'Kiến trúc độc đáo — không thể bỏ qua Đà Lạt.',
            'highlights' => ['Vé vào cổng', 'E-ticket'], 'inclusions' => ['Vé'], 'exclusions' => ['HDV'],
            'notes' => [], 'attrs' => ['duration_hours' => 1, 'activity' => 'architecture']],
        ['code' => 'exp-flower-park', 'cluster' => 'experience', 'category_slug' => 'hoa-linh-thien', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Vườn hoa thành phố Đà Lạt', 'slug' => 'vuon-hoa-thanh-pho-da-lat',
            'price_from' => 50000, 'currency' => 'VND', 'rating' => 4.7, 'review_count' => 445,
            'is_featured' => true, 'location_label' => 'Vườn hoa',
            'summary' => 'Hoa bốn mùa — đẹp nhất cuối tháng 12 (festival).',
            'highlights' => ['Chụp ảnh', 'Festival season'], 'inclusions' => ['Vé'], 'exclusions' => [],
            'notes' => ['Festival có phụ phí khu vực.'], 'attrs' => ['activity' => 'flower']],
        ['code' => 'exp-cau-dat-coffee', 'cluster' => 'experience', 'category_slug' => 'cafe-agri-cau-dat', 'zone_slug' => 'cau-dat',
            'title' => 'Tour nông trại cà phê Cầu Đất + cupping', 'slug' => 'tour-ca-phe-cau-dat-cupping',
            'price_from' => 450000, 'currency' => 'VND', 'rating' => 4.9, 'review_count' => 312,
            'is_featured' => true, 'location_label' => 'Cầu Đất',
            'summary' => 'Farm tour 3h — Arabica specialty, cupping có HDV.',
            'highlights' => ['Cupping', 'Đồi chè'], 'inclusions' => ['HDV cafe', 'Cupping', 'Farm tour'], 'exclusions' => ['Mua cà phê'],
            'notes' => [], 'attrs' => ['duration_hours' => 3, 'activity' => 'coffee_tour']],
        ['code' => 'exp-cu-lan', 'cluster' => 'experience', 'category_slug' => 'ngoai-o-thac-lang', 'zone_slug' => 'ngoai-o-thanh-pho',
            'title' => 'Làng Cù Lần — vé & xe lửa mini', 'slug' => 'lang-cu-lan-ve-xe-lua',
            'price_from' => 150000, 'currency' => 'VND', 'rating' => 4.6, 'review_count' => 234,
            'is_featured' => true, 'location_label' => 'Cù Lần',
            'summary' => 'Làng cổ tích — xe lửa mini, cầu treo, buffet dâu.',
            'highlights' => ['Gia đình', 'Chụp ảnh'], 'inclusions' => ['Vé vào'], 'exclusions' => ['Buffet', 'Xe đón'],
            'notes' => [], 'attrs' => ['duration_hours' => 3, 'activity' => 'village']],
        ['code' => 'exp-clay-tunnel', 'cluster' => 'experience', 'category_slug' => 'ngoai-o-thac-lang', 'zone_slug' => 'ngoai-o-thanh-pho',
            'title' => 'Đường hầm đất sét (Clay Tunnel)', 'slug' => 'duong-ham-dat-set-clay-tunnel',
            'price_from' => 120000, 'currency' => 'VND', 'rating' => 4.5, 'review_count' => 198,
            'is_featured' => false, 'location_label' => 'Clay Tunnel',
            'summary' => 'Diorama đất sét kể chuyện lịch sử Đà Lạt.',
            'highlights' => ['Điêu khắc đất sét', 'Chụp ảnh'], 'inclusions' => ['Vé'], 'exclusions' => [],
            'notes' => [], 'attrs' => ['duration_hours' => 1.5, 'activity' => 'museum']],
        ['code' => 'exp-lumiere', 'cluster' => 'experience', 'category_slug' => 'hoa-linh-thien', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Lumiere Light Garden — vé buổi tối', 'slug' => 'lumiere-light-garden-ve-toi',
            'price_from' => 250000, 'currency' => 'VND', 'rating' => 4.8, 'review_count' => 156,
            'is_featured' => true, 'location_label' => 'Lumiere',
            'summary' => 'Triển lãm ánh sáng — lãng mạn buổi tối.',
            'highlights' => ['18h–22h', 'Couple friendly'], 'inclusions' => ['Vé tối'], 'exclusions' => [],
            'notes' => [], 'attrs' => ['duration_hours' => 2, 'activity' => 'light_show']],
        ['code' => 'exp-cooking-class', 'cluster' => 'experience', 'category_slug' => 'am-thuc-dem', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Lớp nấu ăn Đà Lạt — nửa ngày', 'slug' => 'lop-nau-an-da-lat-nua-ngay',
            'price_from' => 550000, 'currency' => 'VND', 'rating' => 4.7, 'review_count' => 78,
            'is_featured' => false, 'location_label' => 'Trung tâm',
            'summary' => 'Chợ → nấu 3 món (atiso, rau củ cao nguyên).',
            'highlights' => ['Chef HDV', 'Ăn trưa'], 'inclusions' => ['Nguyên liệu', 'Trưa'], 'exclusions' => [],
            'notes' => ['Vegetarian on request.'], 'attrs' => ['duration_hours' => 4, 'activity' => 'cooking']],
        ['code' => 'exp-elephant-falls', 'cluster' => 'experience', 'category_slug' => 'ngoai-o-thac-lang', 'zone_slug' => 'ngoai-o-thanh-pho',
            'title' => 'Thác Voi (Elephant Falls) — tour nửa ngày', 'slug' => 'thac-voi-elephant-falls-nua-ngay',
            'price_from' => 350000, 'currency' => 'VND', 'rating' => 4.7, 'review_count' => 189,
            'is_featured' => true, 'location_label' => 'Thác Voi',
            'summary' => 'Thác 30m phía Nam — ghép Clay Tunnel hoặc riêng.',
            'highlights' => ['View thác', 'HDV'], 'inclusions' => ['Xe', 'Vé', 'HDV'], 'exclusions' => ['Ăn'],
            'notes' => ['Đường đèo.'], 'attrs' => ['duration_hours' => 4, 'activity' => 'waterfall']],
        ['code' => 'exp-tuyenlam-truclam', 'cluster' => 'experience', 'category_slug' => 'tuyen-lam-cap-treo', 'zone_slug' => 'tuyen-lam-truc-lam',
            'title' => 'Tuyền Lâm + Trúc Lâm + cáp treo', 'slug' => 'tuyen-lam-truc-lam-cap-treo',
            'price_from' => 380000, 'currency' => 'VND', 'rating' => 4.8, 'review_count' => 267,
            'is_featured' => true, 'location_label' => 'Tuyền Lâm',
            'summary' => 'Robin Hill cable car, thiền viện, hồ Tuyền Lâm.',
            'highlights' => ['Cáp treo', 'Thiền viện'], 'inclusions' => ['Vé cáp', 'Xe ghép'], 'exclusions' => ['Thuyền'],
            'notes' => [], 'attrs' => ['duration_hours' => 4, 'activity' => 'lake_temple']],
        ['code' => 'exp-robin-hill-cable', 'cluster' => 'experience', 'category_slug' => 'tuyen-lam-cap-treo', 'zone_slug' => 'datanla-robin-hill',
            'title' => 'Vé cáp treo Robin Hill — khứ hồi', 'slug' => 've-cap-treo-robin-hill-khu-hoi',
            'price_from' => 200000, 'currency' => 'VND', 'rating' => 4.7, 'review_count' => 345,
            'is_featured' => false, 'location_label' => 'Robin Hill',
            'summary' => 'View hồ Tuyền Lâm từ cáp treo.',
            'highlights' => ['Khứ hồi', 'E-ticket'], 'inclusions' => ['Vé cáp 2 chiều'], 'exclusions' => [],
            'notes' => [], 'attrs' => ['activity' => 'cable_car']],
        ['code' => 'exp-night-market-food', 'cluster' => 'experience', 'category_slug' => 'am-thuc-dem', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Tour ẩm thực chợ đêm Đà Lạt', 'slug' => 'food-tour-cho-dem-da-lat',
            'price_from' => 380000, 'currency' => 'VND', 'rating' => 4.8, 'review_count' => 298,
            'is_featured' => true, 'location_label' => 'Chợ đêm',
            'summary' => '18h–22h — 7–8 món với HDV am thực.',
            'highlights' => ['Bánh tráng nướng', 'Atiso'], 'inclusions' => ['HDV', 'Tasting'], 'exclusions' => ['Đồ uống thêm'],
            'notes' => [], 'attrs' => ['duration_hours' => 3, 'activity' => 'food_tour']],
        ['code' => 'exp-photo-honeymoon', 'cluster' => 'experience', 'category_slug' => 'hoa-linh-thien', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Tour chụp ảnh couple / trăng mật', 'slug' => 'tour-chup-anh-couple-trang-mat',
            'price_from' => 2200000, 'currency' => 'VND', 'rating' => 4.9, 'review_count' => 67,
            'is_featured' => false, 'location_label' => 'Đà Lạt',
            'summary' => 'Photographer 4h — hồ sương, đồi chè, vườn hoa.',
            'highlights' => ['50+ ảnh chỉnh màu', 'Gợi ý trang phục'], 'inclusions' => ['Xe', 'Photographer'], 'exclusions' => ['Makeup'],
            'notes' => [], 'attrs' => ['duration_hours' => 4, 'activity' => 'photography']],

        // OTHER (7)
        ['code' => 'other-private-guide', 'cluster' => 'other', 'category_slug' => 'huong-dan-rieng', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Hướng dẫn viên riêng (tiếng Anh)', 'slug' => 'huong-dan-vien-rieng-tieng-anh',
            'price_from' => 800000, 'currency' => 'VND', 'rating' => 4.9, 'review_count' => 134,
            'is_featured' => true, 'location_label' => 'Toàn Đà Lạt',
            'summary' => 'HDV bản địa 8h/ngày — linh hoạt lịch trình.',
            'highlights' => ['Tiếng Anh', 'Người Đà Lạt'], 'inclusions' => ['HDV 8h'], 'exclusions' => ['Vé', 'Xe'],
            'notes' => [], 'attrs' => ['service_type' => 'local_guide', 'languages' => ['en']]],
        ['code' => 'other-car-driver', 'cluster' => 'other', 'category_slug' => 'xe-tien-ich', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Xe ô tô có tài xế (theo ngày)', 'slug' => 'xe-o-to-co-tai-xe-theo-ngay',
            'price_from' => 1200000, 'currency' => 'VND', 'rating' => 4.8, 'review_count' => 198,
            'is_featured' => true, 'location_label' => 'Đà Lạt',
            'summary' => '4–7 chỗ + tài ~8h — city tour hoặc Cầu Đất.',
            'highlights' => ['Linh hoạt điểm dừng'], 'inclusions' => ['Xe + lái 8h'], 'exclusions' => ['Vé', 'Xăng ngoài km'],
            'notes' => [], 'attrs' => ['service_type' => 'car_with_driver', 'duration_hours' => 8]],
        ['code' => 'other-wedding-photo', 'cluster' => 'other', 'category_slug' => 'huong-dan-rieng', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Hỗ trợ chụp pre-wedding / cưới ngoại cảnh', 'slug' => 'ho-tro-chup-pre-wedding-da-lat',
            'price_from' => 3500000, 'currency' => 'VND', 'rating' => 4.9, 'review_count' => 45,
            'is_featured' => false, 'location_label' => 'Đà Lạt',
            'summary' => 'Điều phối địa điểm, giấy phép chụp, makeup partner.',
            'highlights' => ['Scout location', '2–3 địa điểm'], 'inclusions' => ['Điều phối', 'Xe'], 'exclusions' => ['Makeup', 'Váy'],
            'notes' => [], 'attrs' => ['service_type' => 'wedding_photo_support']],
        ['code' => 'other-luggage', 'cluster' => 'other', 'category_slug' => 'xe-tien-ich', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Gửi hành lý bến xe / homestay', 'slug' => 'gui-hanh-ly-ben-xe-homestay',
            'price_from' => 80000, 'currency' => 'VND', 'rating' => 4.6, 'review_count' => 76,
            'is_featured' => false, 'location_label' => 'Trung tâm',
            'summary' => 'Gửi vali khi tour ngày — nhận lại chiều.',
            'highlights' => ['Biên nhận'], 'inclusions' => ['1 vali/ngày'], 'exclusions' => [],
            'notes' => [], 'attrs' => ['service_type' => 'luggage_storage']],
        ['code' => 'other-emergency', 'cluster' => 'other', 'category_slug' => 'ho-tro-dac-biet', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Hotline hỗ trợ khẩn cấp 24/7', 'slug' => 'hotline-ho-tro-khan-cap-da-lat',
            'price_from' => 0, 'currency' => 'VND', 'rating' => 5.0, 'review_count' => 38,
            'is_featured' => true, 'location_label' => '24/7',
            'summary' => 'Miễn phí khách đặt qua Hi Đà Lạt — y tế, mất giấy tờ, sương mù.',
            'highlights' => ['VI/EN', 'Không thay 115'], 'inclusions' => ['Hotline'], 'exclusions' => ['Viện phí'],
            'notes' => ['Nguy hiểm tính mạng — gọi 115.'], 'attrs' => ['service_type' => 'medical_assistance', 'price_label' => 'Liên hệ']],
        ['code' => 'other-flower-surprise', 'cluster' => 'other', 'category_slug' => 'ho-tro-dac-biet', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Setup hoa surprise (sinh nhật, cầu hôn)', 'slug' => 'setup-hoa-surprise-da-lat',
            'price_from' => 650000, 'currency' => 'VND', 'rating' => 4.8, 'review_count' => 52,
            'is_featured' => true, 'location_label' => 'Theo địa điểm',
            'summary' => 'Bó hoa + setup nhỏ tại homestay/view hồ.',
            'highlights' => ['Hoa tươi Đà Lạt', 'Bí mật'], 'inclusions' => ['Hoa', 'Setup cơ bản'], 'exclusions' => ['Địa điểm thu phí'],
            'notes' => ['Đặt trước 48h.'], 'attrs' => ['service_type' => 'flower_surprise']],
        ['code' => 'other-motorbike-rental', 'cluster' => 'other', 'category_slug' => 'xe-tien-ich', 'zone_slug' => 'trung-tam-xuan-huong',
            'title' => 'Thuê xe máy Đà Lạt', 'slug' => 'thue-xe-may-da-lat',
            'price_from' => 120000, 'currency' => 'VND', 'rating' => 4.5, 'review_count' => 412,
            'is_featured' => true, 'location_label' => 'Giao tại trung tâm',
            'summary' => 'Tự khám phá — phổ biến đi Cầu Đất, Langbiang.',
            'highlights' => ['Giao homestay', 'Mũ 2 nón'], 'inclusions' => ['Xe/ngày', 'Mũ'], 'exclusions' => ['Xăng', 'CCCD giữ cọc'],
            'notes' => ['Đèo — lái cẩn thận khi sương mù.'], 'attrs' => ['service_type' => 'vehicle_rental'],
            'options' => [['code' => 'xe-so', 'name' => 'Xe số/ngày', 'price_from' => 120000], ['code' => 'xe-tay-ga', 'name' => 'Tay ga/ngày', 'price_from' => 150000]]],
    ],

    'service_listing_faqs' => [
        ['q' => 'Giá "từ" có cố định không?', 'a' => 'Không — giá tham khảo mùa thấp điểm. Hi Đà Lạt chốt giá sau khi nhận ngày và số khách.'],
        ['q' => 'Đặt lưu trú ở Đà Lạt qua Hi Đà Lạt thế nào?', 'a' => 'Xem mục Lưu trú để tham khảo homestay, villa và resort theo khu (trung tâm, Tà Nung, Cầu Đất, Tuyền Lâm) hoặc liên hệ tư vấn viên để được gợi ý phù hợp ngân sách và lịch trình.'],
        ['q' => 'Gộp limousine Sài Gòn + tour + canyoning một đơn?', 'a' => 'Có — một báo giá minh bạch, một đầu mối chăm sóc.'],
        ['q' => 'Canyoning hủy vì mưa?', 'a' => 'Đổi ngày miễn phí hoặc hoàn theo chính sách khi mưa lớn không an toàn.'],
        ['q' => 'Shuttle DLI có chạy đêm không?', 'a' => 'Có phụ phí chuyến bay đêm — đặt trước khi bay.'],
    ],
];

$__companySeed = [
    'name' => 'Hi Đà Lạt',
    'legal_name' => 'Công ty TNHH Du lịch Hi Đà Lạt',
    'tagline' => 'Thành phố ngàn hoa — mùa xuân vĩnh cửu',
    'slogan' => '"Khám phá Đà Lạt như người bản địa"',
    'license_number' => '0045/2024/TCDL-GPLHQT',
    'contact' => [
        'email' => 'hello@hidalat.dev',
        'phone' => '+84 263 388 8888',
        'whatsapp' => '+84 912 888 999',
        'zalo' => '+84 263 388 8888',
        'hotline_label' => 'Hotline',
    ],
    'address' => [
        'street' => 'Số 42 đường 3/2, phường Xuân Hương',
        'locality' => 'Đà Lạt, Lâm Đồng',
        'region' => 'Lâm Đồng',
        'postal' => '670000',
        'country' => 'VN',
    ],
    'social' => [
        'facebook' => ['label' => 'Facebook', 'icon' => 'facebook', 'url' => 'https://www.facebook.com/hidalat'],
        'youtube' => ['label' => 'YouTube', 'icon' => 'play', 'url' => 'https://www.youtube.com/@hidalat'],
        'instagram' => ['label' => 'Instagram', 'icon' => 'photo', 'url' => 'https://www.instagram.com/hidalat'],
        'tiktok' => ['label' => 'TikTok', 'icon' => 'share', 'url' => 'https://www.tiktok.com/@hidalat'],
    ],
    'schema' => [
        'available_language' => ['Vietnamese', 'English', 'Korean'],
        'contact_type' => 'customer service',
        'logo' => null,
    ],
    'footer' => [
        'copyright' => '© :year Hi Đà Lạt. Giấy phép kinh doanh dịch vụ lữ hành số :license.',
        'show_dmca_badge' => true,
    ],
];

return array_merge(
    $__hidalatSeed,
    $__servicesSeed,
    ['company' => $__companySeed],
    ['customize_form' => [
        'destinations_label' => [
            'vi' => 'Bạn muốn khám phá khu vực nào ở Đà Lạt?',




'en' => 'Which areas of Da Lat would you like to explore?',
        ],
        'accommodation_label' => [
            'vi' => 'Bạn thích loại lưu trú nào?',




'en' => 'What kind of stay do you prefer?',
        ],
        'budget_note' => [
            'vi' => 'Ngân sách dự kiến (chưa gồm vé xe/bay Sài Gòn—Đà Lạt)',




'en' => 'Estimated budget (excluding HCMC—Da Lat bus/flight tickets)',
        ],
        'accommodation' => [
            'vi' => [
                'Trung tâm / Hồ Xuân Hương',
                'Tà Nung / view rừng thông',
                'Cầu Đất / farm stay',
                'Villa / resort ven hồ Tuyền Lâm',
                'Nhờ tư vấn giúp tôi',
            ],




'en' => [
                'City centre / Xuan Huong Lake',
                'Ta Nung / pine forest views',
                'Cau Dat / farm stay',
                'Villa / resort near Tuyen Lam',
                'Please advise me',
            ],
        ],
    ]],
    ['nav' => [
        'about_group' => ['vi' => 'Về Hi Đà Lạt',



'en' => 'About Hi Da Lat'],
        'tours' => ['label' => ['vi' => 'Tour',



'en' => 'Tours']],
        'cruise' => [
            'label' => ['vi' => 'Thuyền hồ',



'en' => 'Lake boats'],
            'all_label' => ['vi' => 'Tất cả thuyền hồ',



'en' => 'All lake boats'],
            'all_meta' => ['vi' => 'Thuyền & SUP hồ Tuyền Lâm',



'en' => 'Tuyen Lam boats & SUP'],
            'search_hint' => ['vi' => 'Tour, cà phê, canyoning, cẩm nang…',



'en' => 'Tours, coffee, canyoning, guides…'],
            'search_placeholder' => ['vi' => 'Tìm tour, dịch vụ, bài viết…',



'en' => 'Search tours, services, articles…'],
            'hub_title' => ['vi' => 'Thuyền hồ',



'en' => 'Lake boats'],
            'hub_subtitle' => ['vi' => 'Thuyền picnic, SUP và câu cá hồ Tuyền Lâm.',



'en' => 'Tuyen Lam picnic boats, SUP and fishing trips.'],
        ],
    ]],
    ['listing_hubs' => [
        'tours_hub' => [
            'vi' => ['seo_body' => 'Tour :brand gom city tour, canyoning, Cầu Đất và combo biển — thiết kế bởi chuyên gia bản địa.'],




'en' => ['seo_body' => ':brand tours cover city highlights, canyoning, Cau Dat coffee and beach combos — designed by local experts.'],
        ],
        'cruises_hub' => [
            'vi' => ['seo_body' => 'Thuyền hồ Tuyền Lâm — picnic hoàng hôn, SUP sáng sớm và câu cá từ :brand.'],




'en' => ['seo_body' => 'Tuyen Lam lake boats — sunset picnics, dawn SUP and fishing from :brand.'],
        ],
        'trains_hub' => [
            'vi' => ['seo_body' => 'Xe giường, limousine Sài Gòn/Hà Nội, xe Nha Trang/Mũi Né và shuttle Đà Lạt qua :brand — e-ticket, đổi ngày linh hoạt.'],




'en' => ['seo_body' => 'Sleeper buses, HCMC/Hanoi limousines, Nha Trang/Mui Ne coaches and DLI shuttles via :brand — e-tickets and flexible changes.'],
        ],
        'flights_hub' => [
            'vi' => ['seo_body' => 'Vé bay DLI và đưa đón Liên Khương kết nối tour :brand.'],




'en' => ['seo_body' => 'DLI flights and Lien Khuong transfers aligned with your :brand itinerary.'],
        ],
        'stays_hub' => [
            'vi' => ['seo_body' => ':brand tổng hợp lưu trú Đà Lạt theo khu vực — homestay, villa và resort ở trung tâm, Tà Nung, Cầu Đất và ven hồ Tuyền Lâm. Lọc theo ngân sách, phong cách và gần các điểm trong hành trình của bạn.'],




'en' => ['seo_body' => ':brand brings together Da Lat stays by area — homestays, villas and resorts in the centre, Ta Nung, Cau Dat and Tuyen Lam lakeside. Filter by budget, style and proximity to your itinerary.'],
        ],
        'experiences_hub' => [
            'vi' => ['seo_body' => 'Trải nghiệm Đà Lạt: canyoning, cà phê Cầu Đất, Langbiang, Crazy House, chợ đêm — đặt lẻ hoặc gộp tour qua :brand.'],




'en' => ['seo_body' => 'Da Lat experiences: canyoning, Cau Dat coffee, Langbiang, Crazy House, night market — à la carte or bundled via :brand.'],
        ],
        'extras_hub' => [
            'vi' => ['seo_body' => 'HDV riêng, xe có lái, hoa surprise và hỗ trợ 24/7 trên cao nguyên cùng :brand.'],




'en' => ['seo_body' => 'Private guides, cars with drivers, flower surprises and 24/7 highland support with :brand.'],
        ],
    ]],
);
